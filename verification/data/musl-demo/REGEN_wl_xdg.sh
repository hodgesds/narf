#!/bin/sh
# Rebuild the vendored static-musl `wl_xdg` — a two-process libwayland
# compositor+client exercising the **xdg-shell** window-management protocol
# (xdg_wm_base / xdg_surface / xdg_toplevel). Rung 8 of
# docs/DESKTOP_LINUX_PLAN.md: the protocol every real GUI toolkit (GTK, Qt,
# SDL) uses to map a top-level window — core Wayland only moves pixels.
#
...
# arch-independent).
#
# Output: verification/data/musl-demo/wl_xdg_x86_64 (dynamic-musl PIE).
...
XDG_C="$(dirname "$0")/wl_xdg.c"
musl-gcc -O2 -fPIE -Isrc -I. -I"$FFI_INC" -idirafter /usr/include \
  -c "$XDG_C" -o "$WORK/wl_xdg.o"
musl-gcc -fPIE -pie -mcmodel=large "$WORK/wl_xdg.o" $OBJS "$FFI_A" -lm \
  -o wl_xdg_x86_64

echo "built: $WORK/wayland-$WLVER/wl_xdg_x86_64"
echo "copy it to verification/data/musl-demo/wl_xdg_x86_64"
