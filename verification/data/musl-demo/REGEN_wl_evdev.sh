#!/bin/sh
# Rebuild the vendored static-musl `wl_evdev` — a libwayland compositor that
# bridges **real evdev input** into wl_seat. Rung 11 of
# docs/DESKTOP_LINUX_PLAN.md: it creates a virtual keyboard via /dev/uinput,
# injects a KEY_A, reads it back off the resulting /dev/input/eventN (24-byte
# Linux input_event wire), and forwards it over wl_keyboard to a mapped
# xdg_toplevel client — the hardware-shaped input path a real compositor uses.
#
...
# arch-independent).
#
# Output: verification/data/musl-demo/wl_evdev_x86_64 (dynamic-musl PIE).
...
XDG_C="$(dirname "$0")/wl_evdev.c"
musl-gcc -O2 -fPIE -Isrc -I. -I"$FFI_INC" -idirafter /usr/include \
  -c "$XDG_C" -o "$WORK/wl_evdev.o"
musl-gcc -fPIE -pie -mcmodel=large "$WORK/wl_evdev.o" $OBJS "$FFI_A" -lm \
  -o wl_evdev_x86_64

echo "built: $WORK/wayland-$WLVER/wl_evdev_x86_64"
echo "copy it to verification/data/musl-demo/wl_evdev_x86_64"
