#!/bin/sh
# Rebuild the vendored static-musl `wl_app` — a libwayland compositor that
# fork+execve's the UNMODIFIED weston-simple-shm (/bin/simple_shm, built by
# REGEN_simple_shm.sh) and composites its first real frame. Rung 12 of
# docs/DESKTOP_LINUX_PLAN.md: the first off-the-shelf GUI client on NARF. This
# script builds only the wl_app launcher; run REGEN_simple_shm.sh for the
# client binary it launches.
#
...
# arch-independent).
#
# Output: verification/data/musl-demo/wl_app_x86_64 (dynamic-musl PIE).
...
XDG_C="$(dirname "$0")/wl_app.c"
musl-gcc -O2 -fPIE -Isrc -I. -I"$FFI_INC" -idirafter /usr/include \
  -c "$XDG_C" -o "$WORK/wl_app.o"
musl-gcc -fPIE -pie -mcmodel=large "$WORK/wl_app.o" $OBJS "$FFI_A" -lm \
  -o wl_app_x86_64

echo "built: $WORK/wayland-$WLVER/wl_app_x86_64"
echo "copy it to verification/data/musl-demo/wl_app_x86_64"
