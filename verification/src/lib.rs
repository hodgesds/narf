...
fn smoke_userspace_shared_ring_kick_round_trip() -> TestResult {
    ...
    // matches the mapping a user task would see at the user vaddr)
    // by pushing a Noop into the shared SQ, calling sys_ring_kick
    // synchronously, and reading the Completion back from the
    // shared CQ.
    use alloc::sync::Arc;
    use narf_abi::{NarfStatus, OpCode, Submission, Tag};
    use narf_ipc::{SharedConsumer, SharedProducer, SharedRing};
    use narf_memory::AddressSpace;
    use narf_userspace::{
        install_address_space_lookup, install_core_syscalls, install_global,
    ...
    *USER_AS_SR.lock() = Some(addr_space);
    ...
    // Submit a Noop via the shared SQ (kernel-identity view).
    ...
    // Read the completion back through the shared CQ.
