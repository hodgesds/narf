...
| 4. Compatibility | relibc integration; run standard Rust bins | **closed for the in-tree shell + coreutils path** — `cargo xtask run-interactive` boots QEMU, types `echo hello world` over `-serial stdio`, sees `hello world` echoed back; full IRQ-4 → BYTE_RING → fd 0 → shell → fd 1 → UART chain. `cargo xtask test` runs 5022+ pass / 0 fail / 73 skip. Linux-compat syscalls (epoll/eventfd/timerfd/clone3/mprotect/madvise/statx/signalfd/memfd/mount/chroot/pivot_root/POSIX timers) and `container` namespaces (PID/mount/net/UTS/IPC) land behind Cargo features — see `docs/PERSONAS.md`. Real glibc/musl-linked binaries: blocked on host toolchain + filesystem-backed ld-musl, plumbing landed in Wave 75. |
| 5. Silicon       | Boot on AMD Zen2 Renoir + Phoenix HawkPoint1 | **in progress** — TSC calibration via AMD MSR_PSTATE0 + HPET cross-check landed; FB status-panel for bare-metal diagnostics landed; AMDGPU foundations (PCI + MMIO + ATOMBIOS + IP discovery + GMC/GFX register surfaces) landed; ACPI EC + I²C-HID touchpad + iwlwifi MSI-X / WPA2-PSK / Open Auth landed. Open: real-HW iso-boot proof; bare-metal AMDGPU modeset; real `.ucode` firmware blob plumbed for iwlwifi; AMD Pluton TPM. |
