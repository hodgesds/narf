...
//!                             `#UD`. **REQUIRES a vector-19 IDT
//!                             handler.** NARF has none today; if a
//!                             SIMD FP exception fires (NaN, infinity,
//!                             divide-by-zero in a SSE math op) it
//!                             cascades into `#DF` because the
//!                             unhandled `#XF` itself faults. CI's
//!                             TCG emulation surfaces this where
//!                             KVM-accelerated hosts mask it. So
//!                             this module enables OSFXSR only;
//!                             OSXMMEXCPT lands alongside the `#XF`
//!                             handler.
//!
//! Without OSFXSR, the first user-mode SSE instruction raises `#UD`
...
/// Set CR4.OSFXSR. Idempotent — no-op if already set. Must be called
/// once per CPU after paging is up and before any user-mode entry
/// that could execute SSE.
///
/// `CR4.OSXMMEXCPT` (bit 10) is intentionally **not** flipped here —
/// see the module comment for the missing-#XF-handler rationale.
///
/// # Safety
...
/// Read-back: `true` iff CR4.OSFXSR is set.
#[inline]
