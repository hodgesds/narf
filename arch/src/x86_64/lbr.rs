...
fn n_entries_for_model() -> u8 {
    ...
    if family != 0x06 { return 4; }
    if model >= 0x4E { 32 }
    else if model >= 0x1A { 16 }
    else { 8 }
}
...
pub fn caps() -> LbrCaps {
    ...
    LbrCaps { n_entries: n, from_base: from, to_base: to }
}
...
pub unsafe fn enable(filter: u32) {
    ...
    unsafe { wrmsr(MSR_LBR_SELECT, filter as u64); }
    // SAFETY: same.
    let dc = unsafe { rdmsr(MSR_IA32_DEBUGCTL) };
    // SAFETY: same.
    unsafe { wrmsr(MSR_IA32_DEBUGCTL, dc | DEBUGCTL_LBR); }
}
