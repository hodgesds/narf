...
fn pack(action: u8, target_lpid: u32, payload: u64) -> u64 {
    (action as u64) | ((target_lpid as u64 & 0xFFFFFFFF) << 8) | ((payload & 0xFFFF_FFFF_FFFF) << 8)
    // overlaps w/ target intentionally; hardware-defined
}
...
pub unsafe fn doorbell(mmio_base: usize, action: u8, target_lpid: u32, payload: u64) {
    ...
    // SAFETY: caller-asserted.
    unsafe {
        write_volatile(mmio_base as *mut u64, v);
    }
}
