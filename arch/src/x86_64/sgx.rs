...
pub fn caps() -> SgxCaps {
    ...
    if max < 7 { return SgxCaps::default(); }
    // SAFETY: leaf 7 valid.
    ...
    for sub in 2u32..6 {
        ...
        if typ == 1 {
            ...
            let base = (eax as u64 & 0xFFFF_F000)
                     | ((ebx as u64 & 0xFFFFF) << 32);
            let size = (ecx as u64 & 0xFFFF_F000)
                     | ((edx as u64 & 0xFFFFF) << 32);
            let idx = (sub - 2) as usize;
            if idx < sections.len() {
                sections[idx] = Some(EpcSection {
                    base, size_bytes: size, valid: true,
                });
            }
        }
    }
    ...
    SgxCaps {
        instruction_supported, sgx1, sgx2,
        miscselect, max_size_32, max_size_64,
