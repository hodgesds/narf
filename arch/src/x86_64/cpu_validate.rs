...
pub unsafe fn validate() -> CpuValidation {
    ...
    CpuValidation {
        long_mode, rdtscp, invariant_tsc, nx,
        smep, smap, umip, wrgsbase,
        pcid, x2apic, xsave,
...
pub fn baseline_ok(v: &CpuValidation) -> Result<(), &'static str> {
    if !v.long_mode      { return Err("Long Mode missing"); }
    if !v.rdtscp         { return Err("RDTSCP missing"); }
    if !v.invariant_tsc  { return Err("Invariant TSC missing"); }
    if !v.nx             { return Err("NX missing"); }
    if !v.smep           { return Err("SMEP missing"); }
    if !v.smap           { return Err("SMAP missing"); }
    if !v.wrgsbase       { return Err("FSGSBASE missing"); }
    if !v.xsave          { return Err("XSAVE missing"); }
    if !v.efer_lme_on    { return Err("EFER.LME not enabled"); }
    if !v.efer_nxe_on    { return Err("EFER.NXE not enabled"); }
    if !v.cr4_pae_on     { return Err("CR4.PAE not enabled"); }
    if !v.cr4_osfxsr_on  { return Err("CR4.OSFXSR not enabled"); }
    if !v.cr4_osxsave_on { return Err("CR4.OSXSAVE not enabled"); }
    Ok(())
}
