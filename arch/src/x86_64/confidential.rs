...
fn sev_msr_bits() -> Option<u64> {
    ...
    if max_ext < 0x8000_001F { return None; }
    // SAFETY: leaf valid.
    let (eax, _, _, _) = unsafe { cpuid(0x8000_001F, 0) };
    if eax & (1 << 1) == 0 { return None; } // SEV not supported
...
pub fn detect_environment() -> ConfidentialEnvironment {
    ...
    if let Some(bits) = sev_msr_bits() {
        if bits & SEV_SNP_BIT != 0 { return ConfidentialEnvironment::SevSnpGuest; }
        if bits & SEV_ES_BIT  != 0 { return ConfidentialEnvironment::SevEsGuest; }
        if bits & SEV_BIT     != 0 { return ConfidentialEnvironment::SevGuest; }
    }
...
pub fn host_supports_sme() -> bool {
    ...
    // SAFETY: leaf 0x80000000 always defined.
    let max_ext = unsafe { cpuid(0x8000_0000, 0).0 };
    if max_ext < 0x8000_001F { return false; }
    // SAFETY: leaf valid.
    let (eax, _, _, _) = unsafe { cpuid(0x8000_001F, 0) };
    eax & (1 << 0) != 0
}
