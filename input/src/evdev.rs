...
pub mod key {
    ...
    // ── Full keyboard `KEY_*` set ─────────────────────────────────
    //
    // Values are the exact Linux `include/uapi/linux/input-event-codes.h`
    // constants — the same numbers the i8042 driver emits (scancode-set-1
    // make codes 1..=83 coincide with these for the ASCII cluster). The
    // i2c-HID keyboard driver maps HID Usage-page-0x07 usages onto these
    // codes so a laptop's internal keyboard produces the identical evdev
    // stream a PS/2 keyboard would.
    //
    // Only the codes the HID→KEY_* table references are declared, plus a
    // handful of neighbours for completeness; extend as new usages get
    // mapped. Kept minimal + localized per the coordinate-free constraint
    // with the parallel USB-HID-keyboard work (a merge may union these).
