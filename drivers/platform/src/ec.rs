...
impl AcpiEc {
    ...
    pub fn read_byte(&self, addr: u8) -> Result<u8, DriverError> {
        self.wait_ibf_empty()?;
        unsafe { narf_arch::x86_64::io::out8(self.control_port, EC_CMD_READ); }
        self.wait_ibf_empty()?;
        unsafe { narf_arch::x86_64::io::out8(self.data_port, addr); }
        self.wait_obf_full()?;
    ...
    pub fn write_byte(&self, addr: u8, val: u8) -> Result<(), DriverError> {
        self.wait_ibf_empty()?;
        unsafe { narf_arch::x86_64::io::out8(self.control_port, EC_CMD_WRITE); }
        self.wait_ibf_empty()?;
        unsafe { narf_arch::x86_64::io::out8(self.data_port, addr); }
        self.wait_ibf_empty()?;
        unsafe { narf_arch::x86_64::io::out8(self.data_port, val); }
        Ok(())
    }
}
...
...
pub fn init() {
    *GLOBAL_EC.lock() = Some(AcpiEc::new());
}
