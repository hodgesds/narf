...
checksum types, compressed reads, SINGLE/DUP and RAID0/1/10/5/6 profiles,
...
- Single- and multi-device volumes with **SINGLE, DUP, RAID0, RAID1, RAID10,
  RAID5, and RAID6** chunks. Mount assembles members by FSID and devid from the
...
- RAID1C3/RAID1C4, device add/remove/replace, balance/profile conversion, and
  allocation of new multi-device chunks. Existing RAID0/1/10/5/6 block groups
  are read-write while they have free space; exhausting them returns
  `Unsupported` rather than silently growing an invalid SINGLE chunk.
...
every RAID1/10 mirror, and read-modify-write P/Q for RAID5/6 using Linux's
...
**Single-device chunk growth** (`write::grow_add_chunk`) allocates one new mixed
...
Multi-device chunk growth is not inferred from existing profiles and fails
...
`fixture-raid{0,1,10,5,6}-N.img.sparse` are genuine btrfs-progs 6.17.1 member
images with 64 KiB stripes. Tests assemble each profile, read across stripe
boundaries, perform and remount COW writes, remove each possible member, and
verify newly-written parity; the RAID6 fixture uses RAID6 metadata and exercises
every two-member omission as well.
