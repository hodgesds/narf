...
impl Xhci {
    ...
    pub unsafe fn bring_up(
        ...
        let event_ring = alloc_coherent(4096, DomainId::DRIVER_0)
            .map_err(|_| XhciError::NoMemory)?;
        let er_phys = event_ring.phys_addr().raw();
        let erst = alloc_coherent(4096, DomainId::DRIVER_0)
            .map_err(|_| XhciError::NoMemory)?;
        ...
        unsafe {
            ...
            core::ptr::write_volatile(
                (erst_phys + 8) as *mut u32, ER_SEG_TRBS as u32);
            core::ptr::write_volatile(
                (erst_phys + 12) as *mut u32, 0);
        }
        ...
        // SAFETY: same.
        unsafe { mmio.write32(op_off + OP_USBCMD, USBCMD_RS | USBCMD_INTE); }
        // Wait for HCH = 0.
        ...
        Ok(Self {
            ...
            db_off:  dboff as u64,
            csz_64byte,
            dcbaa, cmd_ring, event_ring, _erst: erst,
            _scratch: scratch,
    ...
    pub fn portsc(&self, port: u8) -> Option<u32> {
        if port == 0 || port > self.caps.max_ports { return None; }
        let off = self.op_off
            + OP_PORTSC_BASE
    ...
    pub fn connected_ports(&self) -> alloc::vec::Vec<(u8, u32)> {
        ...
        for p in 1..=self.caps.max_ports {
            if let Some(v) = self.portsc(p) {
                if v & PORTSC_CCS != 0 { out.push((p, v)); }
            }
        }
        out
    }
    ...
    pub fn port_reset(&self, port: u8) -> Result<u32, XhciError> {
        ...
        if cur & PORTSC_CCS == 0 { return Err(XhciError::BadPort); }
        ...
        let to_write = (cur & !PORTSC_CHG_MASK)
            ...
            | PORTSC_PP;
        // SAFETY: same.
        unsafe { self.mmio.write32(off, to_write); }
    ...
    fn ring_command_doorbell(&self) {
        ...
        unsafe { self.mmio.write32(self.db_off, DB_HC_COMMAND); }
    }
    ...
    fn submit_command(
        &self,
        dword0: u32, dword1: u32, dword2: u32, dword3_no_cycle: u32,
    ) -> Result<(), XhciError> {
        ...
        if *enq_g >= CMD_RING_TRBS - 1 { return Err(XhciError::CmdRingFull); }
    ...
    fn poll_event(&self) -> Option<[u32; 4]> {
        ...
        let d3 = unsafe {
            core::ptr::read_volatile((trb_addr + 12) as *const u32)
        };
        if (d3 & TRB_CYCLE_BIT) != *ccs_g { return None; }
        // SAFETY: same.
        let d0 = unsafe { core::ptr::read_volatile(trb_addr as *const u32) };
        // SAFETY: same.
        let d1 = unsafe {
            core::ptr::read_volatile((trb_addr + 4) as *const u32)
        };
        // SAFETY: same.
        let d2 = unsafe {
            core::ptr::read_volatile((trb_addr + 8) as *const u32)
        };
        *deq_g += 1;
        ...
        let new_deq_phys =
            self.event_ring.phys_addr().raw() + (*deq_g as u64) * 16;
        let ir0 = self.rts_off + IR_BASE_OFF;
        // SAFETY: identity-mapped MMIO.
        unsafe {
            self.mmio.write32(
                ir0 + IR_ERDP_LO,
                (new_deq_phys as u32) | (ERDP_EHB as u32));
            self.mmio.write32(
                ir0 + IR_ERDP_HI, (new_deq_phys >> 32) as u32);
        }
        Some([d0, d1, d2, d3])
    }
    ...
    pub fn drain_events(&self) -> usize {
        let mut n = 0;
        while self.poll_event().is_some() { n += 1; }
        n
    }
    ...
    pub fn address_device(
        ...
        if self.csz_64byte { return Err(XhciError::CmdFailed(0xFE)); }
        ...
        let input  = alloc_coherent(4096, DomainId::DRIVER_0)
            .map_err(|_| XhciError::NoMemory)?;
        let dev_ctx = alloc_coherent(4096, DomainId::DRIVER_0)
            .map_err(|_| XhciError::NoMemory)?;
        let ctrl_tr = alloc_coherent(4096, DomainId::DRIVER_0)
            .map_err(|_| XhciError::NoMemory)?;
        ...
        unsafe {
            core::ptr::write_volatile(
                (input_ctrl + 4) as *mut u32, (1 << 0) | (1 << 1));
        }
    ...
    fn ring_slot_doorbell(&self, slot_id: u8, dci: u32) {
        ...
        unsafe { self.mmio.write32(off, dci); }
    }
    ...
    fn ctrl_enqueue(
        &self,
        slot_id: u8,
        d0: u32, d1: u32, d2: u32, d3_no_cycle: u32,
    ) -> Result<u64, XhciError> {
        let mut g = self.devices.lock();
        let dev = g.get_mut(slot_id as usize)
            .and_then(|s| s.as_mut())
        ...
        compiler_fence(Ordering::SeqCst);
        // SAFETY: same.
        unsafe { core::ptr::write_volatile((trb_addr + 12) as *mut u32, d3); }
        compiler_fence(Ordering::SeqCst);
        dev.ctrl_enq += 1;
        Ok(trb_off)
    }
    ...
    pub fn control_in(
        ...
        let data = alloc_coherent(4096, DomainId::DRIVER_0)
            .map_err(|_| XhciError::NoMemory)?;
        ...
        let setup_d0 = (bm_request_type as u32)
                     | ((b_request as u32) << 8)
                     | ((w_value as u32) << 16);
        let setup_d1 = (w_index as u32)
                     | ((w_length as u32) << 16);
        let setup_d2 = 8u32;
        let trt = if w_length > 0 { TRT_IN_DATA } else { TRT_NO_DATA };
        let setup_d3 = (TRB_TYPE_SETUP_STAGE << TRB_TYPE_SHIFT)
                     | TRB_IDT
                     | (trt << 16);
        ...
        if w_length > 0 {
            let data_d3 = (TRB_TYPE_DATA_STAGE << TRB_TYPE_SHIFT)
                        | TRB_DIR_IN
                        | TRB_IOC;
            self.ctrl_enqueue(
                slot_id,
        ...
        let status_d3 = (TRB_TYPE_STATUS_STAGE << TRB_TYPE_SHIFT)
                     | status_dir
                     | TRB_IOC;
        ...
        for i in 0..copy {
            out[i] = unsafe {
                core::ptr::read_volatile((data_phys + i as u64) as *const u8)
            };
        }
        let _ = data;
        Ok(xferred)
    ...
    pub fn get_device_descriptor(
        ...
        if n < 8 { return Err(XhciError::CmdFailed(0xFB)); }
        Ok(buf)
    }
    ...
    pub fn configure_endpoints(
        ...
        unsafe { core::ptr::write_bytes(input_phys as *mut u8, 0, 4096); }
        ...
        for ep in endpoints.iter().copied() {
            ...
            let tr = alloc_coherent(4096, DomainId::DRIVER_0)
                .map_err(|_| XhciError::NoMemory)?;
            let tr_phys = tr.phys_addr().raw();
            // SAFETY: same.
            unsafe { core::ptr::write_bytes(tr_phys as *mut u8, 0, 4096); }
            ...
            let avg_trb = match ep.kind {
                EndpointKind::IsochIn | EndpointKind::IsochOut =>
            ...
            new_eps.push(EndpointState {
                dci, max_packet: ep.max_packet, kind: ep.kind,
                tr, pcs: 1, enq: 0,
            });
        }
        ...
        let dev_ctx_phys = unsafe {
            ...
            core::ptr::read_volatile(
                (dcbaa_phys + (slot_id as u64) * 8) as *const u64)
        };
        // SAFETY: dev_ctx_phys came from DCBAA which we plant; DMA-mapped.
        let slot_d0 = unsafe {
            core::ptr::read_volatile(dev_ctx_phys as *const u32)
        };
        let new_slot_d0 = (slot_d0 & !(0x1Fu32 << 27))
                        | (max_dci << 27);
        let slot_d1 = unsafe {
            core::ptr::read_volatile((dev_ctx_phys + 4) as *const u32)
        };
        ...
        unsafe {
            core::ptr::write_volatile(
                (input_phys + 4) as *mut u32, add_mask);
        }
        ...
        for ep in new_eps {
            ...
            while dev.eps.len() < need { dev.eps.push(None); }
    ...
    fn ep_enqueue_normal(
        ...
        let d3 = (TRB_TYPE_NORMAL << TRB_TYPE_SHIFT)
               | TRB_IOC
...
pub fn register_pci_driver() {
    ...
    narf_bus::register_pci_driver(narf_bus::PciMatch {
        ...
        kind: narf_bus::MatchKind::Class {
            class: PCI_CLASS_SERIAL_BUS, mask: 0xFF,
        },
        probe,
    });
}
