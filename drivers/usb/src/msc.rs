...
impl From<xhci::XhciError> for MscError {
    fn from(e: xhci::XhciError) -> Self { MscError::Xhci(e) }
}
...
pub fn find_bot_endpoints(
    cfg: &[u8],
) -> Result<(EndpointConfig, EndpointConfig), MscError> {
    let mut i = 0usize;
    ...
    while i + 2 <= cfg.len() {
        ...
        match dtype {
            ...
            5 if len >= 7 && in_match => {
                ...
                if xfer_t == 2 {
                    let kind = if is_in { EndpointKind::BulkIn }
                                  else { EndpointKind::BulkOut };
                    let cfg_ep = EndpointConfig {
                        ep_addr, max_packet: mps, kind,
                    };
                    if is_in  { bulk_in  = Some(cfg_ep); }
                    else      { bulk_out = Some(cfg_ep); }
                }
            }
            _ => {}
        }
...
impl MscDevice {
    ...
    fn read_csw(&self, xhci: &Xhci, expect_tag: u32) -> Result<(u8, u32), MscError> {
        ...
        if n < 13 { return Err(MscError::BadCsw); }
    ...
    fn cmd_no_data(&self, xhci: &Xhci, cb: &[u8]) -> Result<(), MscError> {
        ...
        if status != 0 { return Err(MscError::CswStatus(status)); }
        Ok(())
    }
    ...
    pub fn inquiry(&self, xhci: &Xhci) -> Result<[u8; 36], MscError> {
        ...
        if n < 36 { return Err(MscError::BadLength); }
        Ok(out)
    }
    ...
    pub fn read_capacity_10(&self, xhci: &Xhci) -> Result<(u32, u32), MscError> {
        ...
        let mut out = [0u8; 8];
        let n = self.cmd_data_in(xhci, &cb, &mut out)?;
        if n < 8 { return Err(MscError::BadLength); }
    ...
    pub fn read_block(
        ...
        let mut out: alloc::vec::Vec<u8> =
            alloc::vec::Vec::with_capacity(self.lba_bytes as usize);
        out.resize(self.lba_bytes as usize, 0);
        let n = self.cmd_data_in(xhci, &cb, &mut out[..])?;
        if n != self.lba_bytes as usize { return Err(MscError::BadLength); }
        Ok(out)
    }
