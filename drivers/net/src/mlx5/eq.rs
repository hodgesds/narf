...
pub fn build_create_eq_input(
    params: EqParams,
    pages:  &[u64],
) -> Result<Vec<u8>, EqError> {
    if pages.is_empty()                    { return Err(EqError::NoPages); }
    if params.log_eq_size >= (1 << 5)      { return Err(EqError::BadLogEqSize); }
    if params.uar_page    >= (1 << 24)     { return Err(EqError::BadUarPage); }
    ...
    write_bits_be(&mut out,
        EQC_BIT_LOG_EQ_SIZE, EQC_BIT_LOG_EQ_SIZE_W,
        params.log_eq_size as u64);
    write_bits_be(&mut out,
        EQC_BIT_UAR_PAGE, EQC_BIT_UAR_PAGE_W,
...
pub fn decode_create_eq_input(bytes: &[u8]) -> EqParams {
    EqParams {
        log_eq_size: read_bits_be(bytes,
            EQC_BIT_LOG_EQ_SIZE, EQC_BIT_LOG_EQ_SIZE_W) as u8,
        uar_page: read_bits_be(bytes,
            EQC_BIT_UAR_PAGE, EQC_BIT_UAR_PAGE_W) as u32,
