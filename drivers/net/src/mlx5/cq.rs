...
pub fn build_create_cq_input(params: CqParams, pages: &[u64]) -> Result<Vec<u8>, CqError> {
    if pages.is_empty() {
        return Err(CqError::NoPages);
    }
    if params.log_cq_size >= (1 << 5) {
        return Err(CqError::BadLogCqSize);
    }
    if params.uar_page >= (1 << 24) {
        return Err(CqError::BadUarPage);
    }
...
pub fn decode_create_cq_input(bytes: &[u8]) -> CqParams {
    CqParams {
        log_cq_size: read_bits_be(bytes, CQC_BIT_LOG_CQ_SIZE, CQC_BIT_LOG_CQ_SIZE_W) as u8,
        uar_page: read_bits_be(bytes, CQC_BIT_UAR_PAGE, CQC_BIT_UAR_PAGE_W) as u32,
