...
pub fn build_create_mkey_input(params: MkeyParams, pages: &[u64]) -> Result<Vec<u8>, MkeyError> {
    if pages.is_empty() {
        return Err(MkeyError::NoPages);
    }
    if params.pd >= (1 << 24) {
        return Err(MkeyError::BadPd);
    }
    ...
    out[MKC_OFF_QPN_PD..MKC_OFF_QPN_PD + 4].copy_from_slice(&params.pd.to_be_bytes());
    out[MKC_OFF_START_ADDR..MKC_OFF_START_ADDR + 8]
        .copy_from_slice(&params.start_addr.to_be_bytes());
    out[MKC_OFF_LENGTH..MKC_OFF_LENGTH + 8].copy_from_slice(&params.length.to_be_bytes());
