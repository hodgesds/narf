...
impl FanTable {
    ...
    pub fn parse(raw: &[u8]) -> Result<Self, PpSubtableError> {
        ...
        if raw.len() < 0x37 { return Err(PpSubtableError::Truncated); }
        ...
        Ok(Self {
            ...
            format_revision, content_revision, rev_id,
            thyst:                     raw[5],
            t_min:                     read_u16(6),
            t_med:                     read_u16(8),
            t_high:                    read_u16(10),
...
impl PowerTuneTable {
    ...
    pub fn parse(raw: &[u8]) -> Result<Self, PpSubtableError> {
        ...
        if raw.len() < 0x21 { return Err(PpSubtableError::Truncated); }
        let format_revision  = raw[2];
        let content_revision = raw[3];
        let rev_id = raw[4];
    ...
    /// TDP in whole watts.
    pub fn tdp_watts(&self) -> u16 { self.tdp / 8 }
    /// TjMax (chip junction max temperature) in whole celsius.
    pub fn tj_max_celsius(&self) -> u16 { self.tj_max / 100 }
}
