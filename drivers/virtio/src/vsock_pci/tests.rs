...
use super::{
    VsockDeviceConfig, VsockHdr, VsockOp, VIRTIO_VSOCK_PCI_DEVICE, VIRTIO_VSOCK_PCI_VENDOR,
};
...
kernel_test_in!(
    "drivers/virtio/vsock_pci",
    smoke_virtio_vsock_pci_match_table
);

fn smoke_virtio_vsock_pci_config_decode() -> TestResult {
    let cfg = VsockDeviceConfig {
        guest_cid: 0x0000_0000_0000_0003,
    };
    ...
    let cfg = VsockDeviceConfig {
        guest_cid: 0xCAFE_F00D_DEAD_BEEF,
    };
    let bytes = cfg.encode();
...
kernel_test_in!(
    "drivers/virtio/vsock_pci",
    smoke_virtio_vsock_pci_config_decode
);
...
fn smoke_virtio_vsock_pci_op_values() -> TestResult {
    ...
    if VsockOp::Request as u16 != 1 {
        return TestResult::Fail("REQUEST != 1");
    }
    if VsockOp::Response as u16 != 2 {
        return TestResult::Fail("RESPONSE != 2");
    }
    if VsockOp::Rst as u16 != 3 {
        return TestResult::Fail("RST != 3");
    }
    if VsockOp::Shutdown as u16 != 4 {
        return TestResult::Fail("SHUTDOWN != 4");
    }
    if VsockOp::Rw as u16 != 5 {
        return TestResult::Fail("RW != 5");
    }
    if VsockOp::CreditUpdate as u16 != 6 {
        return TestResult::Fail("CREDIT_UPDATE != 6");
    }
    if VsockOp::CreditRequest as u16 != 7 {
        return TestResult::Fail("CREDIT_REQUEST != 7");
    }
    ...
    if VsockOp::from_raw(0).is_some() {
        return TestResult::Fail("from_raw(0) not None");
    }
    if VsockOp::from_raw(8).is_some() {
        return TestResult::Fail("from_raw(8) not None");
    }
    TestResult::Pass
}
...
fn check_hdr_layout(h: &VsockHdr) -> Option<&'static str> {
    let bytes = h.encode();
    if bytes.len() != VsockHdr::WIRE_SIZE {
        return Some("hdr wire size");
    }
    // Spot-check spec-mandated offsets (§5.10.6).
    if bytes[0x00..0x08] != h.src_cid.to_le_bytes() {
        return Some("src_cid LE @ +0x00");
    }
    if bytes[0x08..0x10] != h.dst_cid.to_le_bytes() {
        return Some("dst_cid LE @ +0x08");
    }
    if bytes[0x10..0x14] != h.src_port.to_le_bytes() {
        return Some("src_port LE @ +0x10");
    }
    if bytes[0x14..0x18] != h.dst_port.to_le_bytes() {
        return Some("dst_port LE @ +0x14");
    }
    if bytes[0x18..0x1C] != h.len.to_le_bytes() {
        return Some("len LE @ +0x18");
    }
    if bytes[0x1C..0x1E] != h.typ.to_le_bytes() {
        return Some("type LE @ +0x1C");
    }
    if bytes[0x1E..0x20] != (h.op as u16).to_le_bytes() {
        return Some("op LE @ +0x1E");
    }
    if bytes[0x20..0x24] != h.flags.to_le_bytes() {
        return Some("flags LE @ +0x20");
    }
    if bytes[0x24..0x28] != h.buf_alloc.to_le_bytes() {
        return Some("buf_alloc LE @ +0x24");
    }
    if bytes[0x28..0x2C] != h.fwd_cnt.to_le_bytes() {
        return Some("fwd_cnt LE @ +0x28");
    }
...
kernel_test_in!(
    "drivers/virtio/vsock_pci",
    smoke_virtio_vsock_pci_hdr_request
);
...
kernel_test_in!(
    "drivers/virtio/vsock_pci",
    smoke_virtio_vsock_pci_hdr_response
);
...
kernel_test_in!(
    "drivers/virtio/vsock_pci",
    smoke_virtio_vsock_pci_hdr_shutdown
);
...
kernel_test_in!(
    "drivers/virtio/vsock_pci",
    smoke_virtio_vsock_pci_hdr_credit_update
);
...
kernel_test_in!(
    "drivers/virtio/vsock_pci",
    smoke_virtio_vsock_pci_hdr_credit_request
);
...
kernel_test_in!(
    "drivers/virtio/vsock_pci",
    smoke_virtio_vsock_pci_hdr_short_slice
);
...
kernel_test_in!(
    "drivers/virtio/vsock_pci",
    smoke_virtio_vsock_pci_live_send_rst
);
