...
use wire::{
    decode_cmd_resp, encode_cmd_req, encode_tmf_req, CmdRespDecoded, VirtioScsiCmdReq,
    VirtioScsiCmdResp, VirtioScsiCtrlTmfReq, CDB_SIZE, SENSE_SIZE,
};
...
impl VirtioScsiPci {
    ...
    pub fn submit_cmd(
        ...
        let req = encode_cmd_req(target, lun, 0xCAFE_BABE, wire::VIRTIO_SCSI_S_SIMPLE, *cdb);
        ...
        unsafe {
            self.notify.write16(off, CMDQ0_IDX);
        }
    ...
    pub fn submit_tmf(
        ...
        // SAFETY: identity-mapped notify region.
        unsafe {
            self.notify.write16(off, CTRLQ_IDX);
        }
        // responsive_spin_until ticks sleep_pumps so cursor/FB stay alive
        // while waiting for the device to publish a used-ring entry.
        let mut q_err = false;
...
static CONTROLLER: IrqSafeSpinLock<Option<VirtioScsiPci>> = IrqSafeSpinLock::new(None);
