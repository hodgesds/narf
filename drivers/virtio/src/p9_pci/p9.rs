...
fn take_str(buf: &[u8], off: &mut usize) -> Result<Vec<u8>, DecodeError> {
    ...
    if buf.len() < *off + n {
        return Err(DecodeError::StringOverflow);
    }
...
impl Header {
    pub fn decode(buf: &[u8]) -> Result<Self, DecodeError> {
        ...
        Ok(Self {
            size,
            kind: buf[4],
            tag: u16::from_le_bytes([buf[5], buf[6]]),
        })
    }
}
...
impl Tattach {
    pub fn encode(&self) -> Vec<u8> {
        let mut body = Vec::with_capacity(4 + 4 + 2 + self.uname.len() + 2 + self.aname.len() + 4);
...
impl Twalk {
    ...
    pub fn decode(buf: &[u8]) -> Result<Self, DecodeError> {
        ...
        if n > Self::MAX_WNAMES {
            return Err(DecodeError::TooManyWnames);
        }
        let mut wnames = Vec::with_capacity(n);
        for _ in 0..n {
            wnames.push(take_str(buf, &mut off)?);
        }
        Ok(Self {
            tag: h.tag,
            fid,
