...
impl VirtioNetPci {
    ...
    pub unsafe fn bring_up(
        ...
        let size_q = |idx: u16| -> Result<(VirtqueueLayout, DmaBuffer, u16, u16), VirtioPciError> {
            ...
            let layout = VirtqueueLayout::new(qsize, phys)
        ...
        unsafe { notify.write16(rx_off, RX_QUEUE); }

        // Allocate the TX scratch.
        let tx_buf = alloc_coherent(4096, DomainId::DRIVER_0)
            .map_err(|_| VirtioPciError::BarMapFailed)?;
    ...
    pub fn tx(&self, frame: &[u8]) -> Result<(), VirtioPciError> {
        ...
        unsafe {
            ...
            for (i, b) in frame.iter().enumerate() {
                core::ptr::write_volatile(
                    (phys + 12 + i as u64) as *mut u8, *b);
            }
        }
        // Two descriptors: header + payload, both device-readable.
        let descs = [
            VirtqDesc { addr: phys,        len: 12,                       flags: 0, next: 0 },
            VirtqDesc { addr: phys + 12,   len: frame.len() as u32,       flags: 0, next: 0 },
        ];
        let head = {
        ...
        unsafe { self.notify.write16(off, TX_QUEUE); }
        let mut spins = 0u32;
        loop {
            let elem = {
                let mut g = self.tx_queue.lock();
...
static CONTROLLER: IrqSafeSpinLock<Option<VirtioNetPci>> =
