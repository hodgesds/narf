//! HID keyboard Scroll Lock LED bridge.
//!
//! See `leds_input_caps.rs` for the full architecture description.
//! This module handles bit 2 of the HID LED byte.
//!
...
use crate::leds_input_caps::{flush_led_byte, HID_LED_BYTE, HID_LED_SCROLL_LOCK};
use crate::triggers::Trigger;

// ── LedScrollLock ─────────────────────────────────────────────────

/// Scroll Lock LED — bridges `/sys/class/leds/input0::scrolllock/` to
/// the keyboard's HID LED output report bit 2.
#[derive(Debug)]
pub struct LedScrollLock {
    name: String,
...
impl LedScrollLock {
    /// Create a new Scroll Lock LED.
    ///
    /// `name` follows the Linux naming convention:
    /// `"input<N>::scrolllock"`.
    pub fn new(name: impl Into<String>) -> Self {
        Self {
            name: name.into(),
            cur_brightness: AtomicU32::new(0),
            trigger: IrqSafeSpinLock::new(Trigger::KeyboardScrollLock),
        }
    }
}

impl LedDevice for LedScrollLock {
    fn name(&self) -> &str {
        &self.name
    }
