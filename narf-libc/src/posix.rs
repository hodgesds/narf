...
/// `bufsiz` bytes.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn readlink(path: *const c_char, buf: *mut c_char, bufsiz: usize) -> ssize_t {
...
pub unsafe extern "C" fn openat(
    ...
    // SAFETY: caller-asserted NUL-terminator.
    let s = unsafe { cstr_to_str(path) };
    match narf_user_runtime::openat(dirfd, s, flags as u64, mode) {
        Some(fd) => fd as c_int,
        None => -1,
    }
}
...
pub unsafe extern "C" fn gethostname(buf: *mut c_char, len: usize) -> c_int {
    ...
    if narf_user_runtime::gethostname(slice) < 0 {
        -1
    } else {
        0
    }
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn getdents64(fd: c_int, dirp: *mut c_void, count: usize) -> ssize_t {
