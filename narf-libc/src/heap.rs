...
// ── brk / sbrk C-ABI shims ─────────────────────────────────────────
//
// The kernel-side `Syscall::Brk` (number 150) is fully implemented:
// arg0 == 0 queries the current break, arg0 != 0 sets it (allocating
// + mapping fresh frames on grow). The C ABI for these calls predates
// mmap and is largely vestigial today — narf-libc's own malloc uses
// mmap, not brk — but POSIX still requires the symbols, and
// relibc-compatible builds expect them present.
//
// Returns:
//   `brk` returns 0 on success, -1 + errno=ENOMEM on failure.
//   `sbrk` returns the *previous* break (current break for query) or
//   `(void*)-1` + errno=ENOMEM on failure.

/// Set the program break to `addr`. Returns 0 on success, -1 if the
/// kernel rejected the request (typically ENOMEM).
///
/// # Safety
/// Lowering `addr` below an in-use heap region invalidates pointers
/// into that region — callers using the libc malloc are insulated
/// (it goes through mmap), but consumers using `brk` directly to
/// bump a custom allocator must not lower past their live data.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn brk(addr: *mut u8) -> i32 {
    let want = addr as usize as u64;
    // SAFETY: SYS_BRK signature: arg0 = new break (0 = query). On
    // grow failure the kernel returns the unchanged break (POSIX
    // brk failure contract); we detect by comparing the returned
    // break to the requested value.
    let got = narf_user_runtime::brk(want as usize) as u64;
    if got == want {
        0
    } else {
...
/// Adjust the program break by `incr` bytes. Returns the *previous*
/// break, or `(void*)-1` on failure. `sbrk(0)` is the canonical
/// "what's the current break" query.
///
/// # Safety
/// `incr` may be negative to release pages; same invariants as
/// [`brk`]. Negative `incr` that crosses below the initial break
/// is undefined.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn sbrk(incr: isize) -> *mut u8 {
    // Query first to get the current break.
    let cur = narf_user_runtime::brk(0);
    if incr == 0 {
        return cur as *mut u8;
    }
    let want = if incr > 0 {
        cur.wrapping_add(incr as usize)
    } else {
        cur.wrapping_sub((-incr) as usize)
    };
    let got = narf_user_runtime::brk(want);
    if got == want {
        cur as *mut u8
    } else {
        crate::errno::set_errno(12); // ENOMEM
