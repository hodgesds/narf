...
**Why NARF doesn't need a separate variant:** *Restated 2026-07 — NARF
now does have a BPF VM (`bpf/`), so the original "no BPF" argument no
longer holds. The conclusion survives on a different basis.*

Tasks Trace RCU exists because a Linux BPF program can be preempted
mid-read and can transiently enter atomic context, so neither RCU nor
SRCU fits. NARF's BPF splits the two cases by construction: an atomic
program cannot await at all (its pointers are QSBR- or
non-preemptible-domain, killed at any await point), and a sleepable
program runs as an ordinary executor task where sleepable-RCU already
applies. There is no program that is simultaneously sleepable and
atomically preemptible, which is exactly the gap Tasks Trace RCU fills.
See `bpf/specification/spec.md` §4.4.
...
3. **No *simultaneously sleepable and atomic* readers → no Tasks Trace
   RCU.** BPF's two execution contexts are disjoint (`bpf/` §4.5), so each
   maps onto QSBR or sleepable-RCU. One fewer variant to implement.
