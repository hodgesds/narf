...
use crate::{
    self as smbios, SmbiosAdditionalInfo, SmbiosBaseboard, SmbiosBattery, SmbiosBios,
    SmbiosBiosLanguage, SmbiosBis, SmbiosBootInfo, SmbiosCache, SmbiosChassis, SmbiosCoolingDevice,
    SmbiosCurrentProbe, SmbiosEventLog, SmbiosFirmwareInventory, SmbiosGroupAssoc,
    SmbiosHwSecurity, SmbiosIpmiDevice, SmbiosMemoryArrayAddr, SmbiosMemoryChannel,
    SmbiosMemoryDevice, SmbiosMemoryDeviceAddr, SmbiosMemoryError32, SmbiosMemoryError64,
    SmbiosMgmtCtrlHci, SmbiosMgmtDevice, SmbiosMgmtDeviceComponent, SmbiosMgmtDeviceThreshold,
    SmbiosOemStrings, SmbiosOnboardExt, SmbiosPhysicalMemoryArray, SmbiosPointingDevice,
    SmbiosPortConnector, SmbiosPowerSupply, SmbiosProcessor, SmbiosProcessorAdditional,
    SmbiosRemoteAccess, SmbiosStringProperty, SmbiosSystem, SmbiosSystemConfig,
    SmbiosSystemPowerControls, SmbiosSystemReset, SmbiosSystemSlot, SmbiosTemperatureProbe,
...
fn smoke_smbios_bios_record() -> TestResult {
    ...
    while buf.len() < fixed_len as usize {
        buf.push(0);
    }
    ...
    if n < 1 {
        return TestResult::Fail("expected ≥ 1 SMBIOS structure");
    }
    let mut out = [SmbiosBios::ZERO; 2];
    let nb = smbios::copy_bios(&mut out);
    if nb != 1 {
        return TestResult::Fail("expected 1 BIOS record");
    }
...
fn smoke_smbios_system_record() -> TestResult {
    ...
    if n != 1 {
        return TestResult::Fail("expected 1 System record");
    }
...
fn smoke_smbios_processor_record() -> TestResult {
    ...
    if n != 1 {
        return TestResult::Fail("expected 1 Processor record");
    }
...
fn smoke_smbios_memory_device() -> TestResult {
    ...
    if n != 1 {
        return TestResult::Fail("expected 1 Memory Device record");
    }
...
fn smoke_smbios_skips_unknown() -> TestResult {
    ...
    buf.push(0);
    buf.push(0xAB);
    while buf.len() - 6 < fixed_len as usize {
        buf.push(0);
    }
    buf.extend_from_slice(b"V\0");
    buf.push(0);
    ...
    let n = smbios::parse_stream(&buf);
    if n < 3 {
        return TestResult::Fail("expected ≥ 3 structures observed");
    }
...
fn smoke_smbios_full_dispatch_coverage() -> TestResult {
    ...
    if smbios::copy_chassis(&mut chassis) != 1 {
        return TestResult::Fail("type 3");
    }
    let mut caches = [SmbiosCache::ZERO; 1];
    if smbios::copy_caches(&mut caches) != 1 {
        return TestResult::Fail("type 7");
    }
    let mut ports = [SmbiosPortConnector::ZERO; 1];
    if smbios::copy_port_connectors(&mut ports) != 1 {
        return TestResult::Fail("type 8");
    }
    let mut slots = [SmbiosSystemSlot::ZERO; 1];
    if smbios::copy_system_slots(&mut slots) != 1 {
        return TestResult::Fail("type 9");
    }
    if smbios::oem_strings().is_none() {
        return TestResult::Fail("type 11");
    }
    if smbios::system_config().is_none() {
        return TestResult::Fail("type 12");
    }
    let mut bl = [SmbiosBiosLanguage::ZERO; 1];
    if smbios::copy_bios_language(&mut bl) != 1 {
        return TestResult::Fail("type 13");
    }
    let mut ga = [SmbiosGroupAssoc::ZERO; 1];
    if smbios::copy_group_assoc(&mut ga) != 1 {
        return TestResult::Fail("type 14");
    }
    let mut el = [SmbiosEventLog::ZERO; 1];
    if smbios::copy_event_log(&mut el) != 1 {
        return TestResult::Fail("type 15");
    }
    let mut pma = [SmbiosPhysicalMemoryArray::ZERO; 1];
    if smbios::copy_physical_memory_arrays(&mut pma) != 1 {
        return TestResult::Fail("type 16");
    }
    let mut me32 = [SmbiosMemoryError32::ZERO; 1];
    if smbios::copy_memory_error32(&mut me32) != 1 {
        return TestResult::Fail("type 18");
    }
    let mut maa = [SmbiosMemoryArrayAddr::ZERO; 1];
    if smbios::copy_memory_array_addrs(&mut maa) != 1 {
        return TestResult::Fail("type 19");
    }
    let mut mda = [SmbiosMemoryDeviceAddr::ZERO; 1];
    if smbios::copy_memory_device_addrs(&mut mda) != 1 {
        return TestResult::Fail("type 20");
    }
    let mut pd = [SmbiosPointingDevice::ZERO; 1];
    if smbios::copy_pointing_devices(&mut pd) != 1 {
        return TestResult::Fail("type 21");
    }
    let mut bat = [SmbiosBattery::ZERO; 1];
    if smbios::copy_batteries(&mut bat) != 1 {
        return TestResult::Fail("type 22");
    }
    let mut sr = [SmbiosSystemReset::ZERO; 1];
    if smbios::copy_system_reset(&mut sr) != 1 {
        return TestResult::Fail("type 23");
    }
    if smbios::hw_security().is_none() {
        return TestResult::Fail("type 24");
    }
    if smbios::system_power_controls().is_none() {
        return TestResult::Fail("type 25");
    }
    let mut vp = [SmbiosVoltageProbe::ZERO; 1];
    if smbios::copy_voltage_probes(&mut vp) != 1 {
        return TestResult::Fail("type 26");
    }
    let mut cd = [SmbiosCoolingDevice::ZERO; 1];
    if smbios::copy_cooling_devices(&mut cd) != 1 {
        return TestResult::Fail("type 27");
    }
    let mut tp = [SmbiosTemperatureProbe::ZERO; 1];
    if smbios::copy_temperature_probes(&mut tp) != 1 {
        return TestResult::Fail("type 28");
    }
    let mut cp = [SmbiosCurrentProbe::ZERO; 1];
    if smbios::copy_current_probes(&mut cp) != 1 {
        return TestResult::Fail("type 29");
    }
    if smbios::remote_access().is_none() {
        return TestResult::Fail("type 30");
    }
    if smbios::bis().is_none() {
        return TestResult::Fail("type 31");
    }
    let mut bi = [SmbiosBootInfo::ZERO; 1];
    if smbios::copy_boot_info(&mut bi) != 1 {
        return TestResult::Fail("type 32");
    }
    let mut me64 = [SmbiosMemoryError64::ZERO; 1];
    if smbios::copy_memory_error64(&mut me64) != 1 {
        return TestResult::Fail("type 33");
    }
    let mut md = [SmbiosMgmtDevice::ZERO; 1];
    if smbios::copy_mgmt_devices(&mut md) != 1 {
        return TestResult::Fail("type 34");
    }
    let mut mdc = [SmbiosMgmtDeviceComponent::ZERO; 1];
    if smbios::copy_mgmt_device_components(&mut mdc) != 1 {
        return TestResult::Fail("type 35");
    }
    let mut mdt = [SmbiosMgmtDeviceThreshold::ZERO; 1];
    if smbios::copy_mgmt_device_thresholds(&mut mdt) != 1 {
        return TestResult::Fail("type 36");
    }
    let mut mc = [SmbiosMemoryChannel::ZERO; 1];
    if smbios::copy_memory_channels(&mut mc) != 1 {
        return TestResult::Fail("type 37");
    }
    if smbios::ipmi_device().is_none() {
        return TestResult::Fail("type 38");
    }
    let mut ps = [SmbiosPowerSupply::ZERO; 1];
    if smbios::copy_power_supplies(&mut ps) != 1 {
        return TestResult::Fail("type 39");
    }
    let mut ai = [SmbiosAdditionalInfo::ZERO; 1];
    if smbios::copy_additional_info(&mut ai) != 1 {
        return TestResult::Fail("type 40");
    }
    let mut oe = [SmbiosOnboardExt::ZERO; 1];
    if smbios::copy_onboard_ext(&mut oe) != 1 {
        return TestResult::Fail("type 41");
    }
    if smbios::mgmt_ctrl_hci().is_none() {
        return TestResult::Fail("type 42");
    }
    if smbios::tpm_device().is_none() {
        return TestResult::Fail("type 43");
    }
    let mut pa = [SmbiosProcessorAdditional::ZERO; 1];
    if smbios::copy_processor_additional(&mut pa) != 1 {
        return TestResult::Fail("type 44");
    }
    let mut fw = [SmbiosFirmwareInventory::ZERO; 1];
    if smbios::copy_firmware_inventory(&mut fw) != 1 {
        return TestResult::Fail("type 45");
    }
    let mut sp = [SmbiosStringProperty::ZERO; 1];
    if smbios::copy_string_properties(&mut sp) != 1 {
        return TestResult::Fail("type 46");
    }
    if smbios::inactive_count() != 1 {
        return TestResult::Fail("type 126");
    }
    let _ = (
        SmbiosOemStrings::ZERO,
        SmbiosSystemConfig::ZERO,
        SmbiosHwSecurity::ZERO,
        SmbiosSystemPowerControls::ZERO,
        SmbiosRemoteAccess::ZERO,
        SmbiosBis::ZERO,
        SmbiosIpmiDevice::ZERO,
        SmbiosMgmtCtrlHci::ZERO,
...
kernel_test_in!(
    "firmware/smbios",
    smoke_smbios_entry_point_rejects_bad_checksum
);
