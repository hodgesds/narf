...
fn decode_sm(bytes: &[u8]) -> Option<EntryPoint> {
    if bytes.len() < 31 || &bytes[0..4] != b"_SM_" {
        return None;
    }
    let ep_len = bytes[5] as usize;
    if ep_len < 31 || bytes.len() < ep_len {
        return None;
    }
    if checksum(&bytes[..ep_len]) != 0 {
        return None;
    }
    if &bytes[16..21] != b"_DMI_" {
        return None;
    }
    if checksum(&bytes[16..32.min(bytes.len())]) != 0 {
        return None;
    }
    let major = bytes[6];
    let minor = bytes[7];
...
pub unsafe fn scan_legacy_bios() -> Option<EntryPoint> {
    ...
    while p + 32 <= END {
        ...
        if &slice[0..5] == b"_SM3_" {
            ...
            if let Some(ep) = decode_sm3(slice) {
                return Some(ep);
            }
        }
        if &slice[0..4] == b"_SM_" {
            if let Some(ep) = decode_sm(slice) {
                return Some(ep);
            }
        }
        p += 16;
...
impl SmbiosPortConnector {
    pub const ZERO: Self = Self {
        internal_designator: [0; 32],
        internal_type: 0,
        external_designator: [0; 32],
        external_type: 0,
        port_type: 0,
    };
}
...
fn parse_bios(t: &mut Tables, fmt: &[u8], pool: &[u8]) {
    if fmt.len() < 9 || t.n_bios >= MAX_BIOS {
        return;
    }
...
fn parse_system(t: &mut Tables, fmt: &[u8], pool: &[u8]) {
    if fmt.len() < 25 || t.n_system >= MAX_SYSTEM {
        return;
    }
...
fn parse_baseboard(t: &mut Tables, fmt: &[u8], pool: &[u8]) {
    if fmt.len() < 9 || t.n_baseboard >= MAX_BASEBOARD {
        return;
    }
...
fn parse_processor(t: &mut Tables, fmt: &[u8], pool: &[u8]) {
    if fmt.len() < 36 || t.n_processor >= MAX_PROCESSORS {
        return;
    }
...
fn parse_memory_device(t: &mut Tables, fmt: &[u8], pool: &[u8]) {
    ...
    if fmt.len() < 28 || t.n_memory >= MAX_MEMORY_DEVICES {
        return;
    }
    ...
    if fmt.len() >= 24 {
        copy_truncated(&mut rec.manufacturer, lookup_string(pool, fmt[23]));
    }
    if fmt.len() >= 25 {
        copy_truncated(&mut rec.serial_number, lookup_string(pool, fmt[24]));
    }
    let i = t.n_memory;
    t.memory[i] = rec;
    t.n_memory = i + 1;
...
fn parse_chassis(t: &mut Tables, fmt: &[u8], pool: &[u8]) {
    if fmt.len() < 9 || t.n_chassis >= MAX_CHASSIS {
        return;
    }
...
fn parse_cache(t: &mut Tables, fmt: &[u8], pool: &[u8]) {
    if fmt.len() < 15 || t.n_cache >= MAX_CACHES {
        return;
    }
    ...
    if fmt.len() > 16 {
        rec.error_correction = fmt[16];
    }
    if fmt.len() > 17 {
        rec.system_cache_type = fmt[17];
    }
    if fmt.len() > 18 {
        rec.associativity = fmt[18];
    }
    let i = t.n_cache;
    t.caches[i] = rec;
    t.n_cache = i + 1;
...
fn parse_port_connector(t: &mut Tables, fmt: &[u8], pool: &[u8]) {
    if fmt.len() < 9 || t.n_port >= MAX_PORT_CONNECTORS {
        return;
    }
...
fn parse_system_slot(t: &mut Tables, fmt: &[u8], pool: &[u8]) {
    if fmt.len() < 13 || t.n_slot >= MAX_SYSTEM_SLOTS {
        return;
    }
...
fn parse_physical_memory_array(t: &mut Tables, fmt: &[u8], _pool: &[u8]) {
    if fmt.len() < 15 || t.n_phys_array >= MAX_PHYSICAL_MEM_ARRAYS {
        return;
    }
    ...
    rec.max_capacity_kb = if max_capacity_32 == 0x8000_0000 && fmt.len() >= 23 {
        ...
        u64::from_le_bytes([
            fmt[15], fmt[16], fmt[17], fmt[18], fmt[19], fmt[20], fmt[21], fmt[22],
        ]) / 1024
    } else {
...
fn parse_memory_error32(t: &mut Tables, fmt: &[u8], _pool: &[u8]) {
    if fmt.len() < 23 || t.n_mem_err32 >= MAX_MEM_ERROR_INFO {
        return;
    }
...
fn parse_mem_array_addr(t: &mut Tables, fmt: &[u8], _pool: &[u8]) {
    if fmt.len() < 15 || t.n_mem_array_addr >= MAX_MEM_ARRAY_ADDRS {
        return;
    }
    ...
    rec.ending_addr_kb = if end_32 == 0xFFFF_FFFF && fmt.len() >= 31 {
        u64::from_le_bytes([
            fmt[23], fmt[24], fmt[25], fmt[26], fmt[27], fmt[28], fmt[29], fmt[30],
        ]) / 1024
    } else {
        end_32 as u64
    };
...
fn parse_mem_device_addr(t: &mut Tables, fmt: &[u8], _pool: &[u8]) {
    if fmt.len() < 19 || t.n_mem_device_addr >= MAX_MEM_DEVICE_ADDRS {
        return;
    }
    ...
    rec.starting_addr_kb = if start_32 == 0xFFFF_FFFF && fmt.len() >= 35 {
        u64::from_le_bytes([
            fmt[19], fmt[20], fmt[21], fmt[22], fmt[23], fmt[24], fmt[25], fmt[26],
        ]) / 1024
    } else {
        start_32 as u64
    };
    rec.ending_addr_kb = if end_32 == 0xFFFF_FFFF && fmt.len() >= 35 {
        u64::from_le_bytes([
            fmt[27], fmt[28], fmt[29], fmt[30], fmt[31], fmt[32], fmt[33], fmt[34],
        ]) / 1024
    } else {
        end_32 as u64
    };
...
fn parse_boot_info(t: &mut Tables, fmt: &[u8], _pool: &[u8]) {
    if fmt.len() < 11 || t.n_boot_info >= MAX_BOOT_INFO {
        return;
    }
...
fn parse_oem_strings(t: &mut Tables, fmt: &[u8], pool: &[u8]) {
    if fmt.len() < 5 || t.n_oem_strings >= 1 {
        return;
    }
...
fn parse_bios_language(t: &mut Tables, fmt: &[u8], pool: &[u8]) {
    if fmt.len() < 22 || t.n_bios_lang >= MAX_BIOS_LANGUAGE {
        return;
    }
...
fn parse_group_assoc(t: &mut Tables, fmt: &[u8], pool: &[u8]) {
    if fmt.len() < 5 || t.n_group_assoc >= MAX_GROUP_ASSOC {
        return;
    }
...
fn parse_event_log(t: &mut Tables, fmt: &[u8], _pool: &[u8]) {
    if fmt.len() < 17 || t.n_event_log >= MAX_EVENT_LOG {
        return;
    }
...
fn parse_pointing_device(t: &mut Tables, fmt: &[u8], _pool: &[u8]) {
    if fmt.len() < 7 || t.n_pointing >= MAX_POINTING_DEVICES {
        return;
    }
...
fn parse_battery(t: &mut Tables, fmt: &[u8], pool: &[u8]) {
    if fmt.len() < 16 || t.n_batteries >= MAX_BATTERIES {
        return;
    }
...
fn parse_system_reset(t: &mut Tables, fmt: &[u8], _pool: &[u8]) {
    if fmt.len() < 13 || t.n_system_reset >= MAX_SYSTEM_RESET {
        return;
    }
...
fn parse_hw_security(t: &mut Tables, fmt: &[u8], _pool: &[u8]) {
    if fmt.len() < 5 || t.n_hw_security >= MAX_HW_SECURITY {
        return;
    }
...
fn parse_system_power_controls(t: &mut Tables, fmt: &[u8], _pool: &[u8]) {
    if fmt.len() < 9 || t.n_sys_power_ctrl >= MAX_SYSTEM_POWER_CTRL {
        return;
    }
...
fn parse_voltage_probe(t: &mut Tables, fmt: &[u8], pool: &[u8]) {
    if fmt.len() < 20 || t.n_voltage >= MAX_VOLTAGE_PROBES {
        return;
    }
...
fn parse_cooling_device(t: &mut Tables, fmt: &[u8], pool: &[u8]) {
    if fmt.len() < 12 || t.n_cooling >= MAX_COOLING_DEVICES {
        return;
    }
    ...
    rec.nominal_speed_rpm =
        u16::from_le_bytes([fmt[12.min(fmt.len() - 1)], fmt[13.min(fmt.len() - 1)]]);
    if fmt.len() > 14 {
...
fn parse_temperature_probe(t: &mut Tables, fmt: &[u8], pool: &[u8]) {
    if fmt.len() < 20 || t.n_temp >= MAX_TEMPERATURE_PROBES {
        return;
    }
...
fn parse_current_probe(t: &mut Tables, fmt: &[u8], pool: &[u8]) {
    if fmt.len() < 20 || t.n_current >= MAX_CURRENT_PROBES {
        return;
    }
...
fn parse_remote_access(t: &mut Tables, fmt: &[u8], pool: &[u8]) {
    if fmt.len() < 6 || t.n_remote_access >= MAX_REMOTE_ACCESS {
        return;
    }
...
fn parse_memory_error64(t: &mut Tables, fmt: &[u8], _pool: &[u8]) {
    if fmt.len() < 31 || t.n_mem_err64 >= MAX_MEM_ERR64 {
        return;
    }
    ...
    rec.mem_array_addr = u64::from_le_bytes([
        fmt[11], fmt[12], fmt[13], fmt[14], fmt[15], fmt[16], fmt[17], fmt[18],
    ]);
...
fn parse_mgmt_device(t: &mut Tables, fmt: &[u8], pool: &[u8]) {
    if fmt.len() < 11 || t.n_mgmt_devices >= MAX_MGMT_DEVICES {
        return;
    }
...
fn parse_mgmt_device_component(t: &mut Tables, fmt: &[u8], pool: &[u8]) {
    if fmt.len() < 11 || t.n_mgmt_dev_comp >= MAX_MGMT_DEVICE_COMP {
        return;
    }
...
fn parse_mgmt_device_threshold(t: &mut Tables, fmt: &[u8], _pool: &[u8]) {
    if fmt.len() < 16 || t.n_mgmt_dev_thresh >= MAX_MGMT_DEVICE_THRESH {
        return;
    }
...
fn parse_memory_channel(t: &mut Tables, fmt: &[u8], _pool: &[u8]) {
    if fmt.len() < 7 || t.n_mem_channels >= MAX_MEMORY_CHANNELS {
        return;
    }
...
fn parse_ipmi(t: &mut Tables, fmt: &[u8], _pool: &[u8]) {
    if fmt.len() < 16 || t.n_ipmi >= MAX_IPMI_DEVICES {
        return;
    }
    ...
    rec.base_address = u64::from_le_bytes([
        fmt[8], fmt[9], fmt[10], fmt[11], fmt[12], fmt[13], fmt[14], fmt[15],
    ]);
    if fmt.len() > 17 {
...
fn parse_power_supply(t: &mut Tables, fmt: &[u8], pool: &[u8]) {
    if fmt.len() < 16 || t.n_power_supplies >= MAX_POWER_SUPPLIES {
        return;
    }
...
fn parse_additional_info(t: &mut Tables, fmt: &[u8], _pool: &[u8]) {
    if fmt.len() < 5 || t.n_additional >= MAX_ADDITIONAL_INFO {
        return;
    }
    let i = t.n_additional;
    t.additional[i] = SmbiosAdditionalInfo {
        entry_count: fmt[4],
    };
    t.n_additional = i + 1;
}
...
fn parse_onboard_ext(t: &mut Tables, fmt: &[u8], pool: &[u8]) {
    if fmt.len() < 11 || t.n_onboard_ext >= MAX_ONBOARD_EXT {
        return;
    }
...
fn parse_mgmt_ctrl_hci(t: &mut Tables, fmt: &[u8], _pool: &[u8]) {
    if fmt.len() < 6 || t.n_mgmt_ctrl_hci >= MAX_MGMT_CTRL_HCI {
        return;
    }
...
fn parse_tpm_device(t: &mut Tables, fmt: &[u8], _pool: &[u8]) {
    if fmt.len() < 32 || t.n_tpm >= MAX_TPM_DEVICES {
        return;
    }
...
fn parse_proc_additional(t: &mut Tables, fmt: &[u8], _pool: &[u8]) {
    if fmt.len() < 7 || t.n_proc_addl >= MAX_PROC_ADDITIONAL {
        return;
    }
    let i = t.n_proc_addl;
...
fn parse_fw_inventory(t: &mut Tables, fmt: &[u8], pool: &[u8]) {
    if fmt.len() < 24 || t.n_fw_inventory >= MAX_FW_INVENTORY {
        return;
    }
...
fn parse_string_property(t: &mut Tables, fmt: &[u8], pool: &[u8]) {
    if fmt.len() < 9 || t.n_string_prop >= MAX_STRING_PROPERTY {
        return;
    }
...
pub fn oem_strings() -> Option<SmbiosOemStrings> {
    let t = DATA.lock();
    if t.n_oem_strings == 0 {
        None
    } else {
        Some(t.oem_strings[0])
    }
}
...
pub fn hw_security() -> Option<SmbiosHwSecurity> {
    let t = DATA.lock();
    if t.n_hw_security == 0 {
        None
    } else {
        Some(t.hw_security[0])
    }
}

pub fn system_power_controls() -> Option<SmbiosSystemPowerControls> {
    let t = DATA.lock();
    if t.n_sys_power_ctrl == 0 {
        None
    } else {
        Some(t.sys_power_ctrl[0])
    }
}
...
pub fn remote_access() -> Option<SmbiosRemoteAccess> {
    let t = DATA.lock();
    if t.n_remote_access == 0 {
        None
    } else {
        Some(t.remote_access[0])
    }
}
...
pub fn mgmt_ctrl_hci() -> Option<SmbiosMgmtCtrlHci> {
    let t = DATA.lock();
    if t.n_mgmt_ctrl_hci == 0 {
        None
    } else {
        Some(t.mgmt_ctrl_hci[0])
    }
}
