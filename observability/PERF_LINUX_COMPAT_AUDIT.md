...
| `perf top` | sampling plus periodic display | partial (x86_64 and aarch64 PMUv3) | Uses the same genuine overflow and mmap-ring path as `perf record`, including CPU-scoped attribution for system-wide events. Symbolized kernel display remains unavailable while the Linux adapter intentionally restricts `/proc/kallsyms` pending capability authorization. |
