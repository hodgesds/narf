...
///
/// Ordinary VMA mutations need exclude only a cross-address-space shared-page
/// migration, so they lock the shard selected by their address-space
/// incarnation. A migration locks every shard in ascending order. This
/// preserves the old global exclusion contract for migration while allowing
/// unrelated processes to publish and retire shared mappings concurrently,
/// matching Linux's per-`mm` mmap-lock scope.
const SHARED_MAPPING_TRANSACTION_SHARDS: usize = 64;

#[repr(align(64))]
struct SharedMappingTransactionShard {
    lock: IrqSafeSpinLock<()>,
}

impl SharedMappingTransactionShard {
    const fn new() -> Self {
        Self {
...
static SHARED_MAPPING_TRANSACTIONS: [SharedMappingTransactionShard;
    SHARED_MAPPING_TRANSACTION_SHARDS] =
    [const { SharedMappingTransactionShard::new() }; SHARED_MAPPING_TRANSACTION_SHARDS];
...
#[inline]
fn shared_mapping_transaction_index(address_space_id: u64) -> usize {
    address_space_id as usize & (SHARED_MAPPING_TRANSACTION_SHARDS - 1)
}

#[inline]
fn lock_address_space_shared_mapping_transaction(
    address_space_id: u64,
) -> narf_lib::sync::IrqSafeSpinLockGuard<'static, ()> {
    SHARED_MAPPING_TRANSACTIONS[shared_mapping_transaction_index(address_space_id)]
        .lock
        .lock()
}

/// Run an ordinary shared-mapping mutation for one address space.
///
/// The closure must not await or recursively enter either shared-mapping
/// transaction helper. Callers take their per-address-space VMA transaction
/// before entering this helper.
pub fn with_address_space_shared_mapping_transaction<R>(
    address_space_id: u64,
    f: impl FnOnce() -> R,
) -> R {
    let _guard = lock_address_space_shared_mapping_transaction(address_space_id);
    f()
}

/// Run a transaction excluding shared-mapping mutations in every address
/// space. This is reserved for operations such as NUMA migration which must
/// replace all aliases of an externally-owned frame atomically.
pub fn with_shared_mapping_transaction<R>(f: impl FnOnce() -> R) -> R {
    let _guards: [_; SHARED_MAPPING_TRANSACTION_SHARDS] =
        core::array::from_fn(|index| SHARED_MAPPING_TRANSACTIONS[index].lock.lock());
    f()
}
...
impl RegionPerms {
    ...
    /// Internal provenance marker for a System V `shmat(2)` VMA. The generic
    /// SHARED bit describes frame ownership but cannot distinguish SysV from
    /// anonymous or file mappings. Syscall teardown uses this marker to invoke
    /// attachment/nattch close accounting only for an overlapping SysV VMA,
    /// mirroring Linux's per-VMA `shm_vm_ops.close`. Bit 18; preserved by VMA
    /// splits, mprotect, mremap, and fork like the other internal flags.
    pub const SYSV_SHM: RegionPerms = RegionPerms(1 << 18);
...
impl Region {
    #[inline]
    fn allows_sparse_backing(&self) -> bool {
        self.perms.contains(RegionPerms::BRK_HEAP)
            || self.perms.contains(RegionPerms::STACK_SEGMENT)
            || self.perms.contains(RegionPerms::FILE_DEMAND)
...
struct RegionTable {
    ...
    /// Next virtual page examined by anonymous CLOCK reclaim.
    ///
    /// Reclaim must rotate through the address space instead of restarting at
    /// its first VMA on every wake.  Once an early prefix has been swapped,
    /// repeatedly walking those zero slots makes sustained pressure quadratic
    /// in RSS and can prevent a faulting process from reaching its timeout or
    /// the OOM path.  The cursor is protected by the region-table lock and is
    /// only an approximate policy hint: VMA mutations may leave it in a hole,
    /// which the ordered scan handles by seeking to the next region.
    #[cfg(target_arch = "x86_64")]
    anon_reclaim_cursor: Cell<u64>,
}
...
fn anonymous_demand_alloc_error(error: crate::FrameAllocError) -> AddressSpaceError {
    match error {
        crate::FrameAllocError::ReservePressure(ticket) => {
            AddressSpaceError::ReclaimPressure(ticket)
        }
        _ => AddressSpaceError::OutOfRange,
    }
}
...
fn mlock_population_error(error: AddressSpaceError) -> AddressSpaceError {
    match error {
        AddressSpaceError::OutOfRange | AddressSpaceError::ReclaimPressure(_) => {
...
impl AddressSpace {
    ...
    /// the outer transaction, then take
    /// [`with_address_space_shared_mapping_transaction`] before calling a
    /// `*_shared_region_locked` method. The lock order is therefore always AS
    /// VMA -> shared-owner shard -> huge table -> regular table.
    ...
    /// Whether any base-page VMA overlapping `[base, base + len)` carries
    /// `perms`, without cloning proportional backing metadata.
    ///
    /// # Safety
    /// The caller must hold [`Self::with_vma_transaction`] while using the
    /// answer to choose a subsequent structural operation.
    pub unsafe fn range_intersects_perms_locked(
        &self,
        base: VirtAddr,
        len: u64,
        perms: RegionPerms,
        ...
        self.regions
            .lock()
            .overlapping_any(lo, hi, |region| region.perms.contains(perms))
    }
    ...
    /// Select the first aligned free interval in the ordinary mmap window
    /// without consuming the monotonic compatibility cursor. Linux uses its
    /// regular unmapped-area search when `mremap(MREMAP_MAYMOVE)` must choose
    /// a destination; doing the same lets repeated moves reuse holes instead
    /// of spreading sparse VMAs and page tables across the high arena.
    ///
    /// # Safety
    /// The caller must hold this address space's VMA transaction continuously
    /// through publication or abandonment of the selected interval.
    pub unsafe fn mmap_unmapped_candidate_locked(
        &self,
        len: u64,
        align: u64,
    ) -> Result<VirtAddr, AddressSpaceError> {
        if len == 0 || len & 0xFFF != 0 || align < 4096 || !align.is_power_of_two() {
            return Err(AddressSpaceError::AlignmentMismatch);
        }
    ...
    pub fn map_region(&self, region: Region) -> Result<(), AddressSpaceError> {
        ...
        let shared_guard = region
            ...
            .then(|| lock_address_space_shared_mapping_transaction(self.identity()));
    ...
    /// transaction, using the mmap high-water mark as the hot candidate and
    /// falling back to the first aligned hole in the mmap window.
    ///
    ...
    pub fn map_private_anonymous_region_anywhere_limited(
        ...
        let selected = {
            ...
            let huge = self.huge_regions.lock();
            let regions = self.regions.lock();
            let caller_hint = hint.as_u64();
            let placement_hint = if caller_hint >= Self::MMAP_CURSOR_BASE {
                caller_hint
    ...
    /// shared it must then hold
    /// [`with_address_space_shared_mapping_transaction`].
    ...
    /// additionally require [`with_address_space_shared_mapping_transaction`].
    ...
    fn map_region_inner(
        ...
        // materialize.
        //
        // Demand-backed heap, stack, file, and ordinary private-anonymous
        // regions may carry a shorter phys list than their page count.
        // Publication/growth records only the VMA length; each page
        // materializes its slot on first fault. Pages past the prefix are
        // demand-zero/file-absent, so teardown has no frame to release.
        let region_pages = region.len >> 12;
        let phys_covers = if region.allows_sparse_backing() {
    ...
    /// [`with_address_space_shared_mapping_transaction`] across the backing
    ...
    pub unsafe fn replace_shared_region_limited_receipt(
        ...
        let _shared_guard = lock_address_space_shared_mapping_transaction(self.identity());
    ...
    /// no-move path). Demand-zero mappings extend only `len`, so the appended
    /// implicit-zero tail faults in like a fresh anonymous `mmap`; mapping
    /// kinds which require an exact scatter list append explicit zero slots.
    /// No copy or extra `materialize` is needed — the original pages keep
    /// their frames at the same virtual address.
    ///
    ...
    fn grow_region_locked(
        ...
        let huge = self.huge_regions.lock();
        let mut regions = self.regions.lock();
        let (region_base, region_len, selected_old_len) = if let Some(expected) = expected_old_len {
            let selected_end = base
                .as_u64()
                .checked_add(expected)
                .ok_or(AddressSpaceError::OutOfRange)?;
            let region = regions
                .containing(base.as_u64())
                .filter(|region| selected_end == region.base.as_u64().saturating_add(region.len))
                .ok_or(AddressSpaceError::Unmapped)?;
            (region.base.as_u64(), region.len, expected)
        } else {
            let region = regions
                .get(base.as_u64())
                .ok_or(AddressSpaceError::Unmapped)?;
            (region.base.as_u64(), region.len, region.len)
        };
        if new_len <= selected_old_len {
            return Ok(None);
        }
        ...
        if regions
            .successor(region_base.saturating_add(1))
            .is_some_and(|successor| successor.base.as_u64() < new_end)
        {
            return Err(AddressSpaceError::Overlap);
        }
        let add_pages = ((new_len - selected_old_len) >> 12) as usize;
        let region = regions
            .get_mut(region_base)
            .ok_or(AddressSpaceError::Unmapped)?;
        ...
        self.bump_mmap_cursor_past(rb, rl);
        Ok(eager_locked.then_some((base.as_u64().saturating_add(selected_old_len), new_end)))
    }
    ...
    /// Move one private base-page interval contained by a single region to a
    /// disjoint virtual range, optionally resizing it at the same time
    /// (`mremap(MREMAP_MAYMOVE)`). Unselected head and tail fragments retain
    /// their original backing offsets.
    ///
    ...
    ///
    /// Linux can also move a source spanning multiple VMA fragments. That
    /// cross-VMA shape remains unsupported rather than silently losing
    /// per-fragment permissions or backing metadata.
    ///
    /// # Safety
    ...
    /// supported private source is one interval contained by one Region;
    /// Linux cross-VMA move-only relocation remains an explicit unsupported
    /// shape.
    ///
    /// # Safety
    /// The caller must hold [`Self::with_vma_transaction`]. If
    /// `shared_transaction_held` is true it must additionally hold
    /// [`with_address_space_shared_mapping_transaction`]; if false, target
    /// classification under the VMA transaction must have proven no shared
    /// overlap. The live-root contract from [`Self::relocate_region`]
    /// continues to apply.
    ...
    fn check_relocation_limits(
        ...
        let source = regions
            .containing(old_lo)
            .filter(|region| old_hi <= region.base.as_u64().saturating_add(region.len))
            .ok_or(AddressSpaceError::Unmapped)?;
    ...
    unsafe fn relocate_region_locked(
        ...
        let source = regions
            ...
            .filter(|region| old_hi <= region.base.as_u64().saturating_add(region.len))
            .ok_or(AddressSpaceError::Unmapped)?;
        let source_region_base = source.base.as_u64();
        let source_region_end = source_region_base
            .checked_add(source.len)
            .ok_or(AddressSpaceError::OutOfRange)?;
        let source_perms = source.perms;
        if new_len > old_len {
            Self::check_mremap_growth_limits_locked(
                &regions,
                &huge,
        ...
        let tail_pages = usize::try_from((source_region_end - old_hi) >> 12)
            .map_err(|_| AddressSpaceError::AllocationFailed)?;
        let head_region = if head_pages == 0 {
            None
        } else {
            Some(Region {
                base: VirtAddr::new(source_region_base),
                len: old_lo - source_region_base,
                perms: source_perms,
                phys: clone_backing(&source.phys, 0, head_pages)?,
            })
        };
        let tail_region = if tail_pages == 0 {
            None
        } else {
            Some(Region {
                ...
                perms: source_perms,
                phys: clone_backing(&source.phys, tail_first, tail_pages)?,
            })
        };
        // Preserve only the materialized prefix for a demand-zero mapping;
        // growth remains an implicit lazy tail instead of allocating one
        // descriptor per virtual page. Other mapping kinds retain their exact
        ...
        regions.try_reserve_nodes(
            1 + usize::from(head_region.is_some()) + usize::from(tail_region.is_some()),
        )?;
        let _source = regions
            .get(source_region_base)
            .filter(|region| region.len == source_region_end - source_region_base)
    ...
    /// [`with_address_space_shared_mapping_transaction`], and `self.root` must
    /// satisfy the live-root contract from [`Self::relocate_region`]. External
    /// file/SysV ownership must be moved in the same outer transaction before
    /// faults can observe a lazy `FILE_DEMAND` destination.
    pub unsafe fn relocate_shared_region_locked_limited(
        ...
        let sparse_file = source_perms.contains(RegionPerms::FILE_DEMAND);
        ...
        let tail_region = (tail_len != 0).then(|| Region {
            base: VirtAddr::new(old_hi),
            len: tail_len,
            perms: source_perms,
            phys: tail_phys,
        ...
        regions.try_reserve_nodes(1 + usize::from(tail_len != 0))?;
    ...
    /// # Safety
    /// The caller must hold [`Self::with_vma_transaction`] followed by
    /// [`with_address_space_shared_mapping_transaction`] and uphold the
    /// live-root contract.
    ...
    /// The caller must hold [`Self::with_vma_transaction`] followed by
    /// [`with_address_space_shared_mapping_transaction`], and `self.root` must
    /// satisfy the live-root contract from [`Self::relocate_region`].
            ...
            if tracks_resident_leaves {
                match mode {
                    ...
                    SharedMremapMode::DontUnmap => {
                        // SAFETY: the validated interval names exactly the
    ...
    pub fn unmap_region(&self, base: VirtAddr) -> Result<Region, AddressSpaceError> {
        ...
        //
        // Shared regions retain the transaction ordering: drop the region
        // lock, acquire this address space's shared-owner shard -> region lock,
        // re-find the mapping, and hold both through TLB invalidation and the
        // owner's release hook below.
        let mut regions = self.regions.lock();
        let shared = regions
            .get(base.as_u64())
            .ok_or(AddressSpaceError::Unmapped)?
        ...
        let _shared_transaction = if shared {
            drop(regions);
            let transaction = lock_address_space_shared_mapping_transaction(self.identity());
    ...
    /// `shared_transaction_held` is true,
    /// [`with_address_space_shared_mapping_transaction`].
    ...
    fn punch_fixed_locked_with_shared_reserving(
        ...
        // `munmap(addr, vma_len)` is overwhelmingly the common case (glibc
        // malloc arenas and short-lived file mappings both use it). Once the
        // exact node and an empty huge/swap intersection are known under the
        // same locks, no split can occur and therefore none of the fallible
        // scratch plan below is needed. This is the same semantic operation
        // as `unmap_region`, kept here so syscall-side file-owner retirement
        // remains in the enclosing VMA transaction.
        let exact_regular = !huge.iter().any(|region| {
            let rb = region.base.as_u64();
            let re = rb.saturating_add(region.len);
            re > lo && rb < hi
        }) && regions.get(lo).is_some_and(|region| region.len == len);
        if exact_regular {
            let region = regions
                .remove(lo)
            ...
            if self.root.as_u64() != 0 {
                ...
                // a replacement until every local leaf has been retired.
                unsafe { self.unmap_region_leaves_local(&region) };
            }
            drop(regions);
            drop(huge);
        ...
        for &key in &overlap_keys {
            ...
            punched_pages = punched_pages
                ...
                .ok_or(AddressSpaceError::OutOfRange)?;
            // A demand-paged region may carry a phys list shorter
    ...
    /// prefix when their first fault lands far from the VMA base; walking a
    /// page-table leaf for every such zero made `munmap` proportional to the
    /// virtual allocation size. Linux's unmap walk skips absent upper-level
    /// tables and consecutive empty PTEs. Grouping the non-zero slots gives
    /// NARF the same shape while retaining the existing range helpers and TLB
    /// ordering for every leaf that can actually be present.
    ///
    /// # Safety
    /// `self.root` must be live and identity-reachable, and `backing` must stay
    /// authoritative for the supplied virtual window through the call.
    #[cfg(any(target_arch = "x86_64", target_arch = "aarch64"))]
    unsafe fn unmap_backing_leaves_local(&self, base: VirtAddr, backing: &[PhysAddr]) {
        let mut cursor = 0usize;
        while cursor < backing.len() {
            while cursor < backing.len() && backing[cursor].raw() == 0 {
                cursor += 1;
            }
            let start = cursor;
            while cursor < backing.len() && backing[cursor].raw() != 0 {
            ...
            let Some(offset) = (start as u64).checked_mul(4096) else {
                return;
            };
            let Some(run_base) = base.as_u64().checked_add(offset) else {
                return;
            };
            let pages = (cursor - start) as u64;
            // SAFETY: the run is page-aligned, names authoritative live
            // backing, and inherits the caller's live-root contract.
            unsafe { self.unmap_virtual_range_local(VirtAddr::new(run_base), pages) };
        }
    }

    /// Tear down every leaf descriptor in an explicit virtual interval. This
    /// is distinct from [`Self::unmap_backing_leaves_local`]: relocation paths
    /// can transfer backing out of their source metadata before retiring its
    /// still-live leaves, so a synthetic empty backing vector cannot describe
    /// whether the source page table is populated.
    ///
    /// # Safety
    /// `self.root` must be live and identity-reachable, and the interval must
    /// remain owned by this address-space transaction through the call.
    #[cfg(any(target_arch = "x86_64", target_arch = "aarch64"))]
    unsafe fn unmap_virtual_range_local(&self, base: VirtAddr, pages: u64) {
        #[cfg(target_arch = "x86_64")]
        {
            // SAFETY: caller supplies the live-root and interval proof.
            let _ = unsafe { crate::x86_64::paging::unmap_4kb_local_range(self.root, base, pages) };
        }
        #[cfg(target_arch = "aarch64")]
        {
            // SAFETY: same contract as the x86_64 arm; the architecture
            // helper supplies the shareable invalidation.
            let _ = unsafe { crate::aarch64::paging::unmap_4kb_range(self.root, base, pages) };
        }
    }
    ...
    fn claim_demand_page(
        ...
        let (index, phys, perms) = {
            ...
            // is valid. A sparse demand region treats a missing prefix entry
            // exactly like an explicit zero slot.
            let phys = region.phys.get(index).copied().unwrap_or(PhysAddr::new(0));
            (index, phys, region.perms)
        };
        if phys.raw() != 0 {
        ...
        // Reserve the authoritative slot before the filesystem hook can take
        // an external page-cache reference. If metadata growth fails, no
        // owner outside memory has observed the fault; once the hook returns,
        // finish_demand_page is allocation-free and can publish or reject it.
        let region = regions
    ...
    fn finish_demand_page(
        ...
        let rb = region.base.as_u64();
        let index = ((v - rb) >> 12) as usize;
        // claim_demand_page reserved this slot before it dropped the lock and
        // entered an allocator or filesystem callback.
    ...
    /// frame.
    ///
    /// Aging is a rotating CLOCK / second-chance pass over the leaf accessed
    /// (A) bits
    ...
    /// A per-address-space virtual cursor resumes after the last inspected page
    /// and wraps once, avoiding repeated scans of an already-swapped prefix.
    #[cfg(target_arch = "x86_64")]
    pub fn collect_anon_reclaim_candidates(
        ...
        {
            let table = self.regions.lock();
            let start = table.anon_reclaim_cursor.get() & !0xFFF;
            let mut next_cursor = start;
            let mut scan_interval = |lo: u64, hi: u64| {
                let first_base = table
                    .by_base
                    .predecessor_or_equal(lo)
                    .filter(|(_, entry)| {
                        lo < entry.region.base.as_u64().saturating_add(entry.region.len)
                    })
                    .map_or(lo, |(base, _)| base);

                for (_, entry) in table.by_base.range(first_base, hi) {
                    let region = &entry.region;
                    ...
                    let rb = region.base.as_u64();
                    let explicit_end = rb.saturating_add((region.phys.len() as u64) * 4096);
                    let begin = lo.max(rb);
                    let end = hi.min(explicit_end);
                    if begin >= end {
                        continue;
                    }
                    let root = self.root;
                    ...
                    while i < last {
                        ...
                        i += 1;
                        while i < last && collected + run_len < max_pages {
                            let cva = rb + (i as u64) * 4096;
                            if region.phys[i].raw() == 0 || table.swap_pages.contains_key(&cva) {
                                break;
                            }
                            next_cursor = cva.saturating_add(4096);
                            // SAFETY: `root` is this space's live identity-reachable root.
                            let cold = unsafe {
                                crate::x86_64::paging::test_and_clear_accessed(
                                    root,
                                    VirtAddr::new(cva),
                                )
                            } == Some(false);
            ...
            let stopped = scan_interval(start, u64::MAX) || (start != 0 && scan_interval(0, start));
            if stopped {
                table
                    .anon_reclaim_cursor
                    .set(if next_cursor < Self::USER_HALF_END {
                        next_cursor
                    } else {
                        0
                    });
    ...
    pub unsafe fn demand_alloc_page(&self, vaddr: VirtAddr) -> Result<(), AddressSpaceError> {
        ...
        if let Some(requests) = swap_requests {
            ...
            return Err(match error {
                crate::swap::SwapError::ReclaimPressure(ticket) => {
                    AddressSpaceError::ReclaimPressure(ticket)
                }
                _ => AddressSpaceError::OutOfRange,
            });
    ...
    fn set_region_flag_range(
        ...
        for region in originals {
            ...
            let head_phys = backing_window(&region.phys, 0, head_pages);
            let middle_phys = backing_window(&region.phys, head_pages, middle_pages);
            let tail_phys = backing_window(
                &region.phys,
                head_pages.saturating_add(middle_pages),
                tail_pages,
            );
    ...
    pub(crate) fn mprotect_range_wx_checked(
        ...
        let touched: Vec<Region> = {
            ...
            for r in originals.into_iter() {
                ...
                // Copy only each fragment's explicit portion of the sparse
                // prefix. Virtual lengths remain derived from the VMA split,
                // never from the number of materialized backing slots.
                let head_phys = backing_window(&r.phys, 0, head_pages);
                let mid_phys = backing_window(&r.phys, head_pages, mid_pages);
                let tail_phys =
                    backing_window(&r.phys, head_pages.saturating_add(mid_pages), tail_pages);
    ...
    pub fn madvise_dontneed(&self, base: VirtAddr, len: u64) -> Result<(), AddressSpaceError> {
        ...
        let discarded_swap;
        {
            ...
            g.for_each_overlapping_mut(lo, hi, |r| {
                ...
                // An implicit sparse tail cannot contain a leaf: faulting one
                // would first extend `phys` through that index while holding
    ...
    /// once, straight into the child's index, while every resident private
    /// page has the child's COW owner retained and the parent leaf
    /// write-protected when this fork's retain is the frame's sole→shared
    /// transition. Pages already shared were made read-only by the fork
    /// that first shared them and are skipped entirely, so repeated forks
    /// never re-touch settled leaves (Linux `copy_page_range`'s wrprotect
    /// shape); absent lazy leaves need no descriptor update. The child's
    /// clones drop the LOCKED bits (Linux clears VM_LOCKED_MASK in the
    /// child), and SHARED regions retain their externally-owned frames
    /// instead of COW owners. Runs of adjacent newly-shared writable pages
    /// are protected with one descriptor walk per run, and a broadcast
    /// fork that restricts more than a small ceiling of pages coalesces
    /// remote invalidation into one full flush.
    ///
    /// `reserve_per_region` publishes through a per-region index
    /// reservation instead of the caller's whole-index reservation, so the
    /// kernel-test failure hook can fail the Nth region's reservation.
    ///
    /// Failure contract: regions already inserted into `child_regions` are
    /// owned by the child — its Drop balances their retains — while the
    /// region being built has exactly its already-retained prefix rolled
    /// back here before the error returns; later regions are untouched.
    ///
    /// # Safety
    /// The caller must hold the VMA transaction, the `parent` region lock,
    /// and the shared-owner transaction; keep every backing frame stable;
    /// and `self.root` must be zero (metadata-only address space) or a
    /// live identity-reachable page-table root.
    #[cfg(any(target_arch = "x86_64", target_arch = "aarch64"))]
    unsafe fn fork_share_regions_into(
        &self,
        parent: &RegionTable,
        child_regions: &mut RegionTable,
        reserve_per_region: bool,
    ) -> Result<(), AddressSpaceError> {
    ...
    /// a shared VMA, it must then hold
    /// [`with_address_space_shared_mapping_transaction`].
    ...
    pub unsafe fn clone_for_fork(&self) -> Result<Self, AddressSpaceError> {
        ...
        let parent_huge: Vec<HugeRegion> = self
            ...
            .collect();
        // Build the child's regions directly from the parent's, in one
        // fused retain/write-protect/publish pass — no intermediate
        // whole-address-space snapshot or side frame vector.
        {
            let mut g = self.regions.lock();
            ...
            // (post-mark) — same vaddr base, phys list, and logical
            // permissions. They already passed alignment, range, overlap,
            // and backing-shape validation in the parent, so they publish
            // through direct reserved insertion. Retaining inside the
            // region transaction keeps munmap from freeing a source before
            // the child's ownership is retained; the injected kernel-test
            // failure path reserves per region so a chosen reservation can
            // fail mid-publication to prove prefix/suffix COW rollback.
            let mut child_regions = child.regions.lock();
            if !fork_child_reserve_failure_injected {
                child_regions.try_reserve_nodes(g.len())?;
            }
            // SAFETY: the VMA transaction, region lock, and shared-owner
            // transaction are held, and `self.root` is live by the function
            // contract.
            unsafe {
                self.fork_share_regions_into(
                    &g,
                    &mut child_regions,
        ...
        // publish shared-owner state and must not run with this IRQ-safe shard
        // held.
        drop(_shared_guard);
    ...
    fn drop(&mut self) {
        ...
        }
        {
            ...
            let _shared_transaction =
                lock_address_space_shared_mapping_transaction(self.identity());
            ...
            #[cfg(target_arch = "x86_64")]
            {
                // A large pressure victim can own hundreds of thousands of
                // swap entries. Teardown must free them without first
                // allocating region-sized VA and SwapPte vectors from the
                // already exhausted heap. Fixed batches preserve one root-lock
                // transaction and one backend retirement per bounded window.
                const SWAP_TEARDOWN_BATCH: usize = crate::swap::SWAP_BATCH_PAGES_MAX;
                let mut pages = [VirtAddr::new(0); SWAP_TEARDOWN_BATCH];
                let mut entries = [crate::swap::SwapPte {
                    swap_type: 0,
                    offset: 0,
                }; SWAP_TEARDOWN_BATCH];
                let mut slots = [crate::swap::SwapSlot::NONE; SWAP_TEARDOWN_BATCH];
                while !regions.swap_pages.is_empty() {
                    let mut count = 0usize;
                    for (&va, state) in regions.swap_pages.iter().rev().take(SWAP_TEARDOWN_BATCH) {
                        assert_eq!(
                            *state,
                            SwapPageState::Swapped,
                            "address space dropped during a swap ownership transition"
                        );
                        pages[count] = VirtAddr::new(va);
                        count += 1;
                    }
                    // SAFETY: Drop has exclusive ownership of this live root;
                    // the stable records remain pinned until removed below.
                    unsafe {
                        crate::swap::take_swap_entries_into(
                            self.root,
                            &pages[..count],
                            &mut entries[..count],
                        )
                    ...
                    for page in &pages[..count] {
                        regions.swap_pages.remove(&page.as_u64());
                    }
                    crate::swap::swap_discard_batch_noalloc(
                        &mut entries[..count],
                        &mut slots[..count],
                    );
                }
...
fn smoke_memory_demand_pressure_is_distinct_from_range_failure() -> TestResult {
    let ticket = crate::reclaim::ReclaimTicket {
        node: 0,
        sequence: 1,
    };
    if anonymous_demand_alloc_error(crate::FrameAllocError::ReservePressure(ticket))
        != AddressSpaceError::ReclaimPressure(ticket)
    {
...
fn smoke_memory_mlock_population_error_is_typed() -> TestResult {
    ...
    if mlock_population_error(AddressSpaceError::OutOfRange) != AddressSpaceError::LockFailed
        || mlock_population_error(AddressSpaceError::ReclaimPressure(ticket))
            != AddressSpaceError::LockFailed
    {
...
fn smoke_memory_file_demand_page_comes_from_the_hook() -> TestResult {
    ...
    let fault_page = vbase + 0x3000;
    TEST_FAULT_PAGE.store(fault_page, Ordering::Relaxed);
    ...
    let served = unsafe { a.demand_alloc_page(VirtAddr::new(fault_page + 0x40)) };
    let slot = a
    ...
    let mut verdict = if served.is_err() {
        ...
        TestResult::Fail("the sparse prefix did not preserve untouched pages")
    } else {
        TestResult::Pass
    };
