...
pub fn connect_in(net_ns_id: u64, remote_addr: [u8; 4], remote_port: u16) -> Result<u32, ()> {
    ...
    //
    // Resolving `iface.gateway` is wrong in both directions. An on-link peer
    // must be ARPed directly, so a directly reachable destination is
    // unreachable whenever the interface's gateway is unset or unresolvable;
    // and a per-route gateway is ignored in favour of a per-interface field
    // that no route need agree with. Because it fails before the SYN is built,
    // it presents as "no SYN emitted" with the routing table and the peer's
    // ARP entry both perfectly correct — which is how it survived a test that
    // was fixed by giving the interface the gateway it expected instead.
