...
impl FieldElement {
    ...
    pub fn mul(&self, rhs: &Self) -> Self {
        ...
        // Final reduction
        let mut rr = [0i64; 5];
        let mut carry = r0 >> 51; rr[0] = (r0 & 0x7ffffffffffff) as i64;
        let v1 = r1 + carry; carry = v1 >> 51; rr[1] = (v1 & 0x7ffffffffffff) as i64;
        let v2 = r2 + carry; carry = v2 >> 51; rr[2] = (v2 & 0x7ffffffffffff) as i64;
        let v3 = r3 + carry; carry = v3 >> 51; rr[3] = (v3 & 0x7ffffffffffff) as i64;
        let v4 = r4 + carry; carry = v4 >> 51; rr[4] = (v4 & 0x7ffffffffffff) as i64;
    ...
    pub fn sqrt(&self) -> Option<Self> {
        ...
        if x.square() == *self { return Some(x); }
        ...
        if xi.square() == *self { return Some(xi); }
        None
    }
}
...
impl Point {
    ...
    pub fn from_bytes_checked(bytes: &[u8; 32]) -> Option<Self> {
        ...
        let mut x = x2.sqrt()?;
        if x.is_negative() != sign { x = FieldElement::ZERO.sub(&x); }
        Some(Point { x, y, z: FieldElement::ONE, t: x.mul(&y) })
    }
    ...
    pub fn to_bytes(&self) -> [u8; 32] {
        ...
        if x.is_negative() { bytes[31] |= 0x80; }
        bytes
    }
}
