//! Tests for cleanroom implementations.
//!
//! Test vectors are drawn from the public-domain primary references so
//! this module remains GPL-free:
//!
//! - SHA-512 / SHA-256 KATs: FIPS 180-4 example appendices
//!   <https://nvlpubs.nist.gov/nistpubs/FIPS/NIST.FIPS.180-4.pdf>
//! - Ed25519 vectors: RFC 8032 §7.1
//!   <https://datatracker.ietf.org/doc/html/rfc8032#section-7.1>
//! - ChaCha20 + Poly1305 vectors: RFC 8439 §2.4.2 / §2.5.2
//!   <https://datatracker.ietf.org/doc/html/rfc8439>
//! - HKDF vectors: RFC 5869 Appendix A
//!   <https://datatracker.ietf.org/doc/html/rfc5869>

use crate::ed25519::{ed25519_public_key, ed25519_sign, ed25519_verify};

#[test]
fn test_ed25519_rfc8032_vector1() {
    let sk = [
        0x9d, 0x61, 0xb1, 0x9d, 0xef, 0xfd, 0x5a, 0x60, 0xba, 0x84, 0x4a, 0xf4, 0x92, 0xec, 0x2c,
        0xc4, 0x44, 0x49, 0xc5, 0x69, 0x7b, 0x32, 0x69, 0x19, 0x70, 0x3b, 0xac, 0x03, 0x1c, 0xae,
        0x7f, 0x60,
    ];
    let expected_sig = [
        0xe5, 0x56, 0x43, 0x00, 0xc3, 0x60, 0xac, 0x72, 0x90, 0x86, 0xe2, 0xcc, 0x80, 0x6e, 0x82,
        0x8a, 0x84, 0x87, 0x7f, 0x1e, 0xb8, 0xe5, 0xd9, 0x74, 0xd8, 0x73, 0xe0, 0x65, 0x22, 0x49,
        0x01, 0x55, 0x5f, 0xb8, 0x82, 0x15, 0x90, 0xa3, 0x3b, 0xac, 0xc6, 0x1e, 0x39, 0x70, 0x1c,
        0xf9, 0xb4, 0x6b, 0xd2, 0x5b, 0xf5, 0xf0, 0x59, 0x5b, 0xbe, 0x24, 0x65, 0x51, 0x41, 0x43,
        0x8e, 0x7a, 0x10, 0x0b,
    ];

    let msg = b"";
    let sig = ed25519_sign(&sk, msg);
    assert_eq!(sig, expected_sig);

    // RFC 8032 §7.1 Test 1 also fixes the public key — round-trip it.
    let expected_pk: [u8; 32] = [
        0xd7, 0x5a, 0x98, 0x01, 0x82, 0xb1, 0x0a, 0xb7, 0xd5, 0x4b, 0xfe, 0xd3, 0xc9, 0x64, 0x07,
        0x3a, 0x0e, 0xe1, 0x72, 0xf3, 0xda, 0xa6, 0x23, 0x25, 0xaf, 0x02, 0x1a, 0x68, 0xf7, 0x07,
        0x51, 0x1a,
    ];
    assert_eq!(ed25519_public_key(&sk), expected_pk);

    // Verify must accept the signature and reject a tampered one — this
    // exercises Point::from_bytes_checked, which depends on the sqrt(-1)
    // constant fix.
    assert!(ed25519_verify(&expected_pk, msg, &sig));
    let mut bad = sig;
    bad[0] ^= 1;
    assert!(!ed25519_verify(&expected_pk, msg, &bad));
}

use crate::sha512::Sha512;

#[test]
fn test_sha512_abc() {
    // FIPS 180-4 SHA-512 example for the message "abc".
    let mut hasher = Sha512::new();
    hasher.update(b"abc");
    let result = hasher.finalize();
    let expected = [
        0xdd, 0xaf, 0x35, 0xa1, 0x93, 0x61, 0x7a, 0xba, 0xcc, 0x41, 0x73, 0x49, 0xae, 0x20, 0x41,
        0x31, 0x12, 0xe6, 0xfa, 0x4e, 0x89, 0xa9, 0x7e, 0xa2, 0x0a, 0x9e, 0xee, 0xe6, 0x4b, 0x55,
        0xd3, 0x9a, 0x21, 0x92, 0x99, 0x2a, 0x27, 0x4f, 0xc1, 0xa8, 0x36, 0xba, 0x3c, 0x23, 0xa3,
        0xfe, 0xeb, 0xbd, 0x45, 0x4d, 0x44, 0x23, 0x64, 0x3c, 0xe8, 0x0e, 0x2a, 0x9a, 0xc9, 0x4f,
        0xa5, 0x4c, 0xa4, 0x9f,
    ];
    assert_eq!(result, expected);
}

#[test]
fn test_sha256_abc() {
    // FIPS 180-4 SHA-256 example for the message "abc".
    use crate::sha256::Sha256;
    let mut hasher = Sha256::new();
    hasher.update(b"abc");
    let result = hasher.finalize();
    let expected = [
        0xba, 0x78, 0x16, 0xbf, 0x8f, 0x01, 0xcf, 0xea, 0x41, 0x41, 0x40, 0xde, 0x5d, 0xae, 0x22,
        0x23, 0xb0, 0x03, 0x61, 0xa3, 0x96, 0x17, 0x7a, 0x9c, 0xb4, 0x10, 0xff, 0x61, 0xf2, 0x00,
        0x15, 0xad,
    ];
    assert_eq!(result, expected);
}

#[test]
fn test_chacha20_rfc8439_block() {
    // RFC 8439 §2.3.2 test vector: key/nonce/counter with known block.
    use crate::chacha20::{chacha20_block, chacha20_init};
    let key: [u8; 32] = [
        0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e,
        0x0f, 0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17, 0x18, 0x19, 0x1a, 0x1b, 0x1c, 0x1d,
        0x1e, 0x1f,
    ];
    let nonce: [u8; 12] = [
        0x00, 0x00, 0x00, 0x09, 0x00, 0x00, 0x00, 0x4a, 0x00, 0x00, 0x00, 0x00,
    ];
    let state = chacha20_init(&key, &nonce, 1);
    let mut out = [0u8; 64];
    chacha20_block(&state, &mut out);
    let expected = [
        0x10, 0xf1, 0xe7, 0xe4, 0xd1, 0x3b, 0x59, 0x15, 0x50, 0x0f, 0xdd, 0x1f, 0xa3, 0x20, 0x71,
        0xc4, 0xc7, 0xd1, 0xf4, 0xc7, 0x33, 0xc0, 0x68, 0x03, 0x04, 0x22, 0xaa, 0x9a, 0xc3, 0xd4,
        0x6c, 0x4e, 0xd2, 0x82, 0x64, 0x46, 0x07, 0x9f, 0xaa, 0x09, 0x14, 0xc2, 0xd7, 0x05, 0xd9,
        0x8b, 0x02, 0xa2, 0xb5, 0x12, 0x9c, 0xd1, 0xde, 0x16, 0x4e, 0xb9, 0xcb, 0xd0, 0x83, 0xe8,
        0xa2, 0x50, 0x3c, 0x4e,
    ];
    assert_eq!(out, expected);
}

#[test]
fn test_poly1305_rfc8439() {
    // RFC 8439 §2.5.2: Poly1305 over the canonical 34-byte message.
    use crate::poly1305::Poly1305;
    let key: [u8; 32] = [
        0x85, 0xd6, 0xbe, 0x78, 0x57, 0x55, 0x6d, 0x33, 0x7f, 0x44, 0x52, 0xfe, 0x42, 0xd5, 0x06,
        0xa8, 0x01, 0x03, 0x80, 0x8a, 0xfb, 0x0d, 0xb2, 0xfd, 0x4a, 0xbf, 0xf6, 0xaf, 0x41, 0x49,
        0xf5, 0x1b,
    ];
    let msg = b"Cryptographic Forum Research Group";
    let mut mac = Poly1305::new(&key);
    mac.update(msg);
    let tag = mac.finalize();
    let expected = [
        0xa8, 0x06, 0x1d, 0xc1, 0x30, 0x51, 0x36, 0xc6, 0xc2, 0x2b, 0x8b, 0xaf, 0x0c, 0x01, 0x27,
        0xa9,
    ];
    assert_eq!(tag, expected);
}

#[test]
fn test_chacha20_poly1305_aead_roundtrip() {
    // RFC 8439 §2.8.2 — seal then open round trip with the canonical
    // 114-byte plaintext, 12-byte AAD, and per-message nonce.
    use crate::aead::{chacha20_poly1305_open, chacha20_poly1305_seal};
    let key: [u8; 32] = [
        0x80, 0x81, 0x82, 0x83, 0x84, 0x85, 0x86, 0x87, 0x88, 0x89, 0x8a, 0x8b, 0x8c, 0x8d, 0x8e,
        0x8f, 0x90, 0x91, 0x92, 0x93, 0x94, 0x95, 0x96, 0x97, 0x98, 0x99, 0x9a, 0x9b, 0x9c, 0x9d,
        0x9e, 0x9f,
    ];
    let nonce: [u8; 12] = [
        0x07, 0x00, 0x00, 0x00, 0x40, 0x41, 0x42, 0x43, 0x44, 0x45, 0x46, 0x47,
    ];
    let aad = b"\x50\x51\x52\x53\xc0\xc1\xc2\xc3\xc4\xc5\xc6\xc7";
    let plaintext: &[u8] = b"Ladies and Gentlemen of the class of '99: \
        If I could offer you only one tip for the future, sunscreen would be it.";

    let mut buf = plaintext.to_vec();
    let tag = chacha20_poly1305_seal(&key, &nonce, aad, &mut buf);
    assert_ne!(&buf[..], plaintext, "seal must overwrite plaintext with ciphertext");

    // Tag tamper → open rejects, ciphertext stays sealed.
    let mut tampered = buf.clone();
    let mut bad_tag = tag;
    bad_tag[0] ^= 1;
    assert!(!chacha20_poly1305_open(&key, &nonce, aad, &mut tampered, &bad_tag));

    // AAD tamper → open rejects.
    let mut tampered = buf.clone();
    assert!(!chacha20_poly1305_open(&key, &nonce, b"wrong-aad", &mut tampered, &tag));

    // Honest open → plaintext recovered.
    assert!(chacha20_poly1305_open(&key, &nonce, aad, &mut buf, &tag));
    assert_eq!(&buf[..], plaintext);
}

#[test]
fn test_hkdf_sha256_rfc5869_case1() {
    // RFC 5869 Appendix A.1 (HKDF-SHA-256 Test Case 1).
    use crate::hkdf::{hkdf_expand, hkdf_extract};
    let ikm = [0x0bu8; 22];
    let salt = [
        0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b, 0x0c,
    ];
    let info = [0xf0u8, 0xf1, 0xf2, 0xf3, 0xf4, 0xf5, 0xf6, 0xf7, 0xf8, 0xf9];
    let prk = hkdf_extract(Some(&salt), &ikm);
    let expected_prk = [
        0x07, 0x77, 0x09, 0x36, 0x2c, 0x2e, 0x32, 0xdf, 0x0d, 0xdc, 0x3f, 0x0d, 0xc4, 0x7b, 0xba,
        0x63, 0x90, 0xb6, 0xc7, 0x3b, 0xb5, 0x0f, 0x9c, 0x31, 0x22, 0xec, 0x84, 0x4a, 0xd7, 0xc2,
        0xb3, 0xe5,
    ];
    assert_eq!(prk, expected_prk);

    let okm = hkdf_expand(&prk, &info, 42);
    let expected_okm = [
        0x3c, 0xb2, 0x5f, 0x25, 0xfa, 0xac, 0xd5, 0x7a, 0x90, 0x43, 0x4f, 0x64, 0xd0, 0x36, 0x2f,
        0x2a, 0x2d, 0x2d, 0x0a, 0x90, 0xcf, 0x1a, 0x5a, 0x4c, 0x5d, 0xb0, 0x2d, 0x56, 0xec, 0xc4,
        0xc5, 0xbf, 0x34, 0x00, 0x72, 0x08, 0xd5, 0xb8, 0x87, 0x18, 0x58, 0x65,
    ];
    assert_eq!(okm.as_slice(), &expected_okm[..]);
}

#[test]
fn test_base_mul_one() {
    use crate::curve25519::Point;
    let mut scalar = [0u8; 32];
    scalar[0] = 1;
    let p = Point::BASE.mul(&scalar);
    assert_eq!(p.to_bytes(), Point::BASE.to_bytes(), "BASE*1 should equal BASE");
}

#[test]
fn test_base_double() {
    use crate::curve25519::Point;
    // 2*B compressed (well-known): see e.g. RFC 8032 §6.
    let expected: [u8; 32] = [
        0xc9, 0xa3, 0xf8, 0x6a, 0xae, 0x46, 0x5f, 0x0e,
        0x56, 0x51, 0x38, 0x64, 0x51, 0x0f, 0x39, 0x97,
        0x56, 0x1f, 0xa2, 0xc9, 0xe8, 0x5e, 0xa2, 0x1d,
        0xc2, 0x29, 0x23, 0x09, 0xf3, 0xcd, 0x60, 0x22,
    ];
    assert_eq!(Point::BASE.double().to_bytes(), expected);
}
