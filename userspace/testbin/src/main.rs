...
fn run_probes_x86_64(rsp_at_entry: u64) {
    ...
    // ── fb probe ──────────────────────────────────────────────
    // Connect to the active scanout, query its geometry (spot-check
    // the format), map the draw-ring, enqueue a Fill, disconnect.
    // Layout of the SharedRing<DrawCmd, 16> backing page:
    //
    ...
    //   Tags: 1 = FILL, 2 = FLUSH.
    //
    // The testbin lives in a disjoint workspace and can't pull
    // narf-ipc / narf-fb directly, so the bytes go in by hand.
    let mut fb_ok = false;
    // SAFETY: pure syscalls; bad handle → fb_ok stays false.
    unsafe {
        let h = rt::fb_connect(0);
        if h != 0 {
            // Geometry sanity check — accepted any non-zero w/h.
            let info_ok = rt::fb_info(h)
                .map(|i| i.width > 0
                      && i.height > 0
                      && i.format == rt::FB_FORMAT_XRGB8888)
                .unwrap_or(false);
            if info_ok {
                let va = rt::fb_ring_map(h);
                if !va.is_null() {
