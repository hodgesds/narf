...
//!
//! Values match Linux `<asm-generic/errno-base.h>` and `<asm-generic/errno.h>`.
//!
//! Syscall handlers return negated error values (`-errno` as `u64` with status Ok).
//! Helper functions [`to_wire`] and [`to_ret`] convert positive errno constants
//! into their return register representations.
