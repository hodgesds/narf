...
{
    // `copy_mnt_ns` stamps the UNSHARING task's user namespace onto the new
    // mount namespace, which is what `mount(2)`'s CAP_SYS_ADMIN check is
...
pub(crate) fn ns_owner_for(
    ...
    {
        crate::namespaces::user_ns_of(task)
            .map(|ns| ns as alloc::sync::Arc<dyn narf_filesystem::NsOwner>)
    }
...
/// instance.
/// A queued signal's externally visible generic fields:
/// `(si_code, si_value, si_pid)`.
type SigqueuePayload = (i32, u64, u32);
/// Internal queued siginfo. `poll_band` is explicit provenance for signals
/// generated by fasync/SIGIO: positive `si_code` values overlap with other
/// siginfo unions (notably CLD_EXITED..CLD_DUMPED), so the code alone cannot
/// identify the SIGPOLL layout.
#[derive(Clone, Copy)]
pub(crate) struct QueuedSiginfo {
    pub(crate) code: i32,
    pub(crate) value: u64,
    pub(crate) pid: u32,
    pub(crate) poll_band: Option<u32>,
}
...
impl QueuedSiginfo {
    ...
    const fn sigpoll(code: i32, fd: i32, band: u32) -> Self {
        Self {
            code,
            value: fd as u32 as u64,
            pid: 0,
            poll_band: Some(band),
        }
    }

    const fn public_fields(self) -> SigqueuePayload {
        (
            self.code,
            self.value,
            match self.poll_band {
                Some(band) => band,
                None => self.pid,
            },
        )
...
type SigwaitTaken = (u32, Option<QueuedSiginfo>);
type SigqueueMap = BTreeMap<(u64, u32), alloc::collections::VecDeque<QueuedSiginfo>>;
...
pub(crate) fn store_sigqueue_info_depth(
    ...
    if signum < SIGRT_QUEUE_MIN {
        ...
        q.clear();
        q.push_back(QueuedSiginfo::generic(si_code, si_value, si_pid));
...
pub(crate) fn sigqueue_store_and_raise_bit(
    ...
    sigqueue_store_payload_and_raise_bit(
        task,
        signum,
        QueuedSiginfo::generic(si_code, si_value, si_pid),
    )
}

fn sigqueue_store_payload_and_raise_bit(
    task: u64,
    signum: u32,
    payload: QueuedSiginfo,
) -> Option<(usize, bool)> {
    let mut g = SIGQUEUE_INFO[sigqueue_bucket(task)].values.lock();
    let m = g.get_or_insert_with(BTreeMap::new);
    let queued: usize = m.range((task, 0)..=(task, 64)).map(|(_, q)| q.len()).sum();
...
pub(crate) fn take_sigqueue_info(task: u64, signum: u32) -> Option<SigqueuePayload> {
    ...
    v.map(QueuedSiginfo::public_fields)
}
...
pub(crate) fn peek_sigqueue_info(task: u64, signum: u32) -> Option<QueuedSiginfo> {
...
/// (inner).
pub(crate) fn sigqueue_take_and_clear(task: u64, signum: u32) -> Option<QueuedSiginfo> {
    let mut g = SIGQUEUE_INFO[sigqueue_bucket(task)].values.lock();
    let payload = g.as_mut().and_then(|m| {
        let q = m.get_mut(&(task, signum))?;
        let v = q.pop_front();
...
/// Snapshot the live members of an internal process-group id. SIGIO's
/// deferred readiness callback uses the same membership walk as kill(2), so a
/// task that joins or leaves after F_SETOWN is reflected at delivery time.
pub(crate) fn pgrp_task_snapshot(pgrp: u64) -> alloc::vec::Vec<u64> {
    if pgrp == 0 {
        return alloc::vec::Vec::new();
    }
    ...
    if !targets.contains(&pgrp) && read_pgid(pgrp) == pgrp {
        // `read_pgid` defaults an arbitrary number to itself, so require an
        // actual task/mapping (or the currently-running harness task) before
        // treating that number as a group leader.
        if crate::task::task_get(pgrp).is_some()
            || task_to_pid_raw(pgrp).is_some()
            || pgrp == current_task_id()
        {
            targets.push(pgrp);
        }
    }
...
/// Queue one Linux SIGIO notification in task context.
///
/// A configured signal carries the SIGPOLL union: `si_code` is POLL_IN/etc
/// (or SI_SIGIO when that would be ambiguous), with explicit `si_band` and
/// `si_fd` metadata. The signal-frame builder recognizes that shape below.
...
pub(crate) fn raise_sigio_pending(
    ...
    if configured_signal == 0 {
        ...
        if let Some((_depth, was_empty)) =
            sigqueue_store_and_raise_bit(task, SIGIO, 0x80, 0, 0)
        {
            signal_raise_notify(task, was_empty);
        }
        return;
    }
    // Linux cannot expose POLL_IN..POLL_HUP as positive si_codes on signals
    // that define their own positive code namespace: the values would be
    // ambiguous (for example POLL_IN == CLD_EXITED). fs/fcntl.c therefore
    // reports SI_SIGIO while retaining the SIGPOLL si_band/si_fd union.
    let signal_has_specific_codes = matches!(configured_signal, 4 | 5 | 7 | 8 | 11 | 17 | 29 | 31);
    let si_code = if configured_signal != SIGIO && signal_has_specific_codes {
        SI_SIGIO
    } else {
        poll_code
    };
    signal_stopcont_interaction(task, configured_signal);
    match sigqueue_store_payload_and_raise_bit(
        task,
        configured_signal,
        QueuedSiginfo::sigpoll(si_code, fd, poll_band),
    ) {
        Some((_depth, was_empty)) => signal_raise_notify(task, was_empty),
        None => {
            // Linux falls through to do_send_sig_info(SIGIO, SEND_SIG_PRIV)
            // when the configured realtime signal cannot be queued.
            signal_stopcont_interaction(task, SIGIO);
            if let Some((_depth, was_empty)) =
                sigqueue_store_and_raise_bit(task, SIGIO, 0x80, 0, 0)
            {
...
fn default_signal_delivery_restricted_active(
    ...
    let (si_code, si_value, si_pid, si_addr) =
        queued.map_or((0, 0, 0, 0), |info| match info.poll_band {
            Some(band) => {
                // The arch frame writers select si_pid for non-positive codes
                // and si_addr for positive codes. Both fields overlap the
                // SIGPOLL si_band slot, so provide the band through whichever
                // route the Linux-compatible si_code selects.
                (info.code, info.value, band, u64::from(band))
            }
